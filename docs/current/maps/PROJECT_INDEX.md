...
| Experience VR platform antenna | [`technical/VR_PLATFORM_ANTENNA_MODEL.md`](../technical/VR_PLATFORM_ANTENNA_MODEL.md), then the progress-floor, Rune Stone and runtime models linked there | Target-only P0 model; later bounded implementation must use current sector actors and the existing Asterion/platform rotation seam |
| Experience VR platform energy VFX for Rune installation or Asterion drive | [`technical/VR_PLATFORM_ENERGY_VFX_MODEL.md`](../technical/VR_PLATFORM_ENERGY_VFX_MODEL.md), then the Rune or progress-floor model for source ownership | Future `PlatformEnergyVfxProjection` and `src/xr/vfx/createVrPlatformEnergyVfxActor.js`; current read-only evidence is in Rune installation and Asterion gyro interactions |
...
| Experience VR Rune Stones, sector vessels or rune transport/install | [`technical/VR_RUNE_STONES_MODEL.md`](../technical/VR_RUNE_STONES_MODEL.md), then [`technical/VR_SCENARIO_DIRECTOR_MODEL.md`](../technical/VR_SCENARIO_DIRECTOR_MODEL.md); read the current runtime and handoff before implementation work | Rune domain is **PARTIALLY IMPLEMENTED** through A9.5. Socket capture and persistent installed truth are implemented; installed-stone collision, Ether flow and authored Scenario after `4.80` remain future bounded work. |
...
- [`technical/VR_RUNE_STONES_MODEL.md`](../technical/VR_RUNE_STONES_MODEL.md) — canonical model and implementation boundary for the partially implemented five-pair Rune domain, including tuning, targetability, transport and installation readiness.
...
- [`technical/VR_PLATFORM_ANTENNA_MODEL.md`](../technical/VR_PLATFORM_ANTENNA_MODEL.md) — canonical target contract and P0–P9 implementation route for the not-yet-implemented WOOD/FIRE/EARTH platform antenna.
