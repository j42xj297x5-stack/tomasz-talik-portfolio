...
| Experience VR Rezonator Asterionowy, sector control, late Rune act and finale | [`technical/VR_ASTERION_RESONATOR_MODEL.md`](../technical/VR_ASTERION_RESONATOR_MODEL.md), then [`concept/EXPERIENCE_VR_RUNES_RESONATOR_FINALE.md`](../concept/EXPERIENCE_VR_RUNES_RESONATOR_FINALE.md), the Rune Stone and progress-floor models | Current target separates sandbox mechanics from Scenario and replaces the historical antenna model; runtime implementation remains future. |
| Experience VR platform energy VFX for Zwornik reveal, Rune installation or Asterion drive | [`technical/VR_PLATFORM_ENERGY_VFX_MODEL.md`](../technical/VR_PLATFORM_ENERGY_VFX_MODEL.md), then the Rune or progress-floor model for source ownership | Future shared `PlatformEnergyVfxActor`; `RUNE_BINDER_REVEAL`, `RUNE_INSTALL` and `FLOOR_DRIVE` remain read-only presentation profiles. |
...
| Experience VR Rune Stones, Zworniki or rune transport/install | [`technical/VR_RUNE_STONES_MODEL.md`](../technical/VR_RUNE_STONES_MODEL.md), then [`technical/VR_ASTERION_RESONATOR_MODEL.md`](../technical/VR_ASTERION_RESONATOR_MODEL.md); read Scenario docs only for dramaturgy/Guidance ownership | Natural Rune A9 foundation is implemented through A9.6. Scenario after `4.80`, Ether/Water special flow, target Zwornik presentation, Rune audio, Rezonator and finale remain future; Zwornik spin is superseded. |
