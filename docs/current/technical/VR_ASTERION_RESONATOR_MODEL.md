...
Runtime R2A implements installed-Rune-powered acquisition and transient `SECTOR LOCK`; R2B implements controller-driven EARTH/WOOD/FIRE motion and committed `0/1/2/3` detents, while the Metal extension implements two independently committed transient DOFs and composed sector motion. R4 implements the event-driven Resonator Field Actor: derived `resonatorExists`, immutable runtime `α/β/γ` descriptor, POWERED/FIELD-ACTIVE and partial/full/symmetric/asymmetric coarse-field semantics with bounded read-only queries and exactly-on-change subscriptions.

The volumetric sector acquisition beam, bounded Platform Energy extension, revised aligned nominal field/presentation, generic registered-target containment, per-target resonance, sign/ring response, and Large Glyph Astrolabium eligibility are **IMPLEMENTED**. The runtime now requires family knowledge alone for ordinary early Large Glyph pulls and family knowledge plus `PULL_READY` only at `SPHERE_FAR`. The five Large Glyph nodes are registered, and their late motion includes approximately `0.02 rad/s` angular motion plus deterministic independent `20–110 m` radial oscillation over approximately `135 s`, with stable per-target phase offsets owned by the Large Glyph Actor. Discovery Guidance and the authored late sequence through stable `5.60` are implemented. Metal dual-DOF control, descriptor/shape contribution, morph and rounding are implemented; the new canonical `0 / 20% / 50% / 75%` gameplay containment expansion is **TARGET / NOT YET RUNTIME-SYNCHRONIZED**. Water advanced control, Water synchronization and the finale remain **FUTURE / NOT IMPLEMENTED**; the late-motion values remain runtime **TUNING** and hardware/perceptual QA remains outstanding.

This document is the primary CURRENT technical model of the Asterion Resonator. It freezes the sandbox, Scenario, Guidance, Rune Binder, implemented local sector-control and implemented Field-domain boundaries. The specialized [`VR_ASTERION_RESONATOR_FIELD_MODEL.md`](VR_ASTERION_RESONATOR_FIELD_MODEL.md) is its subordinate CURRENT Field sub-model, not an alternative canon. Implemented R2A/R2B/R4, Field Frame/presentation, target acquisition/response and Astrolabium eligibility seams are established runtime contracts. API naming remains open only for genuinely future audio, WATER synchronization, later target classes and finale work.
...
GRIP nie przejmuje sektora natychmiast. Sektorowy strumień musi trafiać ten sam legalny, zasilony sektor nieprzerwanie przez pełne **1.0 s**; dopiero wtedy powstaje **SECTOR LOCK** i ruch kontrolera może sterować lokalnym sektorem. Zmiana celu albo utrata legalnego trafienia przed upływem 1.0 s zeruje acquisition timer. Przed lockiem Kula nie steruje lokalnym sektorem; po locku implemented sector-control mode interpretuje ruch względem przejętego sektora, nie całej platformy. Pierwszy valid frame nowej próby (`no active candidate → ACQUIRING`) emituje osobny presentation seam i natychmiast odtwarza family-specific `electricity_short_01–05`; jest to dźwięk pierwszego kontaktu elektrycznego, nie potwierdzenie `LOCKED`. Utrata kontaktu i reacquisition lub bezpośrednia zmiana candidate przed lockiem tworzy nową próbę bez dodatkowego cooldownu. Pełna mapa pięciu rodzin jest kanoniczna. Physical control is implemented for EARTH/FIRE/WOOD and for Metal’s two transient DOFs; Water physical control remains future.
...
The lightweight 16-corner cage, translucent skin, curved skeleton, rounded fillets, and configuration morphing remain approved. Nominal aperture coordinates now create the primary shape; wall bow is secondary presentation tuning. Runtime resolves this revised geometry in a dedicated Resonator Field Frame, mounts the presentation through that frame, and composes FIRE outward radial alignment with `entryDirection`. The Resonator's long-range supported-object response is its family-colored Proto-Astro sign plus up to three thin resonance rings; no scoring mechanic or generic halo is added to that acquisition presentation. After ring 3 grants `PULL_READY`, a `SPHERE_FAR` Large Glyph recovers its actor-owned authored, light-reactive materials. If an ordinary close-range ray subsequently targets it, the target reuses the existing normal `VrTargetHalo` and glyph PointLight feedback; that halo is neither a long-range response nor another Resonator acquisition stage.
...
## 7a. CURRENT Guidance and Scenario path
...
`CHECK_RESONATOR_JOIN` handles the opposite event order. The authored continuation is `5.10 → 5.20 → 5.30 → 5.40 → 5.50 → 5.60 → 100.10`, with stable implemented boundary `5.60`. Scenario recognizes physical results without gating their creation. `P6 → 5.10` and `P5 → 4.80` are debug/QA aliases only. Entry to `5.60` owns the full-array acknowledgement and semantic `CAN_USE_ADVANCED_RESONATOR`; that capability does not gate Metal mechanics.
...
`PULL_READY` is a transient late pull grant, not family knowledge. In ordinary early gameplay a learned Large Glyph family is sufficient without Resonator; only a late `SPHERE_FAR` escaped/reacquisition target additionally requires `PULL_READY`. This early/late distinction and late-stage predicate are **IMPLEMENTED CURRENT**. The future final Haiku Cosmos rule adds Water synchronization contact to this late gate; it does not create a Scenario point ID and is not yet implemented.

At `SPHERE_FAR`, each Large Glyph remains on its black, light-insensitive material set until its live acquisition state reaches `PULL_READY`. Readiness restores that exact target's existing authored materials and permits the existing ordinary close-range halo and PointLight feedback; decay from three rings to two immediately restores the black presentation and removes this late ray-feedback eligibility. This presentation projection does not grant crystal creation, which remains independently progression-gated.

## 8a. Late motion and Metal — CONTROL IMPLEMENTED / PERCENTAGE EXPANSION TARGET; Water — FUTURE

At `SPHERE_FAR`, Large Glyph Actor owns approximately `0.02 rad/s` angular motion and deterministic independent `20–110 m` radial oscillation over approximately `135 s`. These values remain tuning and hardware QA is outstanding.

Installed natural Rune truth powers the matching sector. Therefore an installed Metal Rune makes the Metal acquisition beam, lock and control legal without `CAN_USE_ADVANCED_RESONATOR` or point `5.60`. `M00` is powered but field-inactive and adds no range; any positive Metal DOF activates its field contribution.

Metal implements `M(angle, tilt)` as two independently committed transient DOFs with levels `0/1/2/3`, physical detents `0°/13°/23°/36°`, dominant-axis gesture arbitration and composed physical sector motion. The canonical mapping is `angle → LATERAL`, `tilt → FORWARD`; Metal never expands VERTICAL. Both DOFs map levels `0/1/2/3` monotonically to expansion fractions `0.00 / 0.20 / 0.50 / 0.75`, so greater deflection always increases coverage.

For angle fraction `p`, each existing EARTH/WOOD half-extent scales independently from the center axis: `resolvedLeftHalfExtent = baseLeftHalfExtent × (1 + p)` and `resolvedRightHalfExtent = baseRightHalfExtent × (1 + p)`. This preserves asymmetric apertures. Thus the balanced `13 m + 13 m` core resolves from `26.0 m` total width at level `0` to `31.2 / 39.0 / 45.5 m` at levels `1/2/3`.

For tilt fraction `p`, the FIRE band's boundaries resolve as `lerp(baseNear, 10 m, p)` and `lerp(baseFar, 130 m, p)`. NEAR therefore progresses `10–50 → 10–66 → 10–90 → 10–110 m`; MID progresses `50–90 → 42–98 → 30–110 → 20–120 m`; FAR progresses `90–130 → 74–130 → 50–130 → 30–130 m`. The authoritative details live in the Field sub-model.

`M22` remains the future harmonic center in control-space/synchronization semantics, but now contributes `+50%` LATERAL expansion and `50%` interpolation toward the global depth boundaries. The current runtime still implements the superseded fixed-meter/zero-M22 model; synchronizing runtime behavior is a separate code task.

The same Metal state enters the descriptor and `resolveAsterionResonatorFieldShape()`, so presentation and `containsPointInAsterionResonatorField()` share one nominal resolved shape. There is no separate visual field. Fillet/bow/skin/skeleton/morph remain presentation-only. Metal rounding multiplies mismatch fillet by `1.50` at M22, `0.90` with exactly one off-center DOF and `0.60` with both off-center, finally clamped to `0.32`.

Exact gesture axes/signs, dominance margin, composed-motion comfort and rounding readability are **TUNING / HARDWARE QA**. Hardware smoke confirms Metal control after the flat Shell identity fix and working acquisition after Metal installation, not final feel or the pending percentage-expansion runtime synchronization.

The complete Ether path is also implemented: fourth natural installation, mandatory Monkey intervention, `CAN_TUNE_ETHER_RUNE`, `VI + VO → VU`, separate `etherRuneTuned`, physical `VU` reveal/transport, Monkey capture, persistent Water readiness override, Water Binder readiness, ordinary Water natural installation, five-natural crossing and semantic advanced unlock at `5.60`. `V` remains special and outside natural collections.

Water angle/tilt control, hue/luminance, W22 presentation, harmonic recognition, Water Sync Lock/Contact, Haiku damping and final anti-bypass pull are binding **FUTURE / NOT IMPLEMENTED** canon.
...
| Astrolabium Tuning Actor | implemented read-only derivation of band family truth and bounded eligibility | duplicated progression, recipes, Resonator timers, Scenario |
...
| Metal Resonator extension | implemented transient DOFs, motion, descriptor/shape expansion and rounding projection | Scenario gating, target family, harmonic recognition |
| future Water advanced domain | future hue/luminance, Sync Lock and bounded Sync Contact truth | Large Glyph motion, acquisition stages/timers, target family |
| Large Glyph Actor | implemented angular and radial late motion; future damped response | Water synchronization truth, pull permission |
...
Poza runtime i **NOT IMPLEMENTED** pozostają: Water control/hue/luminance/pulse, Water Sync Lock/Contact, harmonic recognition, Haiku slowdown and final Water-specific anti-bypass eligibility, final Water hunt/card, world dissolution, XR finale and genuinely future audio APIs. Metal control/expansion/rounding, Ether intervention/capture, Water readiness override and natural Water installation through `5.60` are **IMPLEMENTED CURRENT**. Metal numeric and perceptual tuning remains outstanding.
...
The bounded read-only level/angle snapshot and exactly-once semantic `DETENT_COMMITTED` subscription seam are implemented. The sector acquisition beam is implemented as described below. Resonator target audio is implemented as an observer projection: global rotating AIM loops cover incomplete field-entry acquisition attempts, while each target owns at most one rotating LOCK lifecycle from ring three until retained rings expire; both are moving 140 m spatial emitters. Spark VFX and WATER motion/Field contribution are not implemented; Metal motion and Field contribution are implemented. Revised Field presentation, physical target containment/resonance/response and Astrolabium eligibility are implemented. R4's Field Actor and immutable analytic descriptor runtime are implemented from installed Rune truth and R2B committed levels; transient angles do not contribute.
