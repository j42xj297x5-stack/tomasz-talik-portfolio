...
The volumetric sector acquisition beam, bounded Platform Energy extension, revised aligned nominal field/presentation, generic registered-target containment, per-target resonance, sign/ring response, and Large Glyph Astrolabium eligibility are **IMPLEMENTED**. The runtime now requires family knowledge alone for ordinary early Large Glyph pulls and family knowledge plus `PULL_READY` only at `SPHERE_FAR`. The five Large Glyph nodes are registered, and their late motion includes approximately `0.02 rad/s` angular motion plus deterministic independent `20–110 m` radial oscillation over approximately `135 s`, with stable per-target phase offsets owned by the Large Glyph Actor. Discovery Guidance and the Scenario semantic join `4.80 → 5.10` are implemented, but `5.10` has no authored continuation. Field/target audio, METAL/WATER/ETHER contribution, Water synchronization and the finale remain **FUTURE / NOT IMPLEMENTED**; the late-motion values remain runtime **TUNING** and hardware/perceptual QA remains outstanding.
...
## 8a. Late motion — IMPLEMENTED; advanced Metal/Water — BINDING TARGET / NOT IMPLEMENTED

At `SPHERE_FAR`, Large Glyph Actor currently owns approximately `0.02 rad/s` angular/spherical motion and deterministic independent radial oscillation over approximately `20–110 m`: `radius(t) = 65 m + 45 m * sin(2πt / T + phase)`. `T ≈ 135 s` and stable deterministic per-target phase offsets are implemented runtime tuning; hardware QA remains outstanding. The motion crosses NEAR/MID/FAR. The three-wing core can still acquire it, but tracking one FIRE band becomes difficult.
...
- **METAL = LATERAL/FORWARD EXPANSION PLUS HARMONIC SHAPE.** One Metal DOF controls LATERAL expansion and the other controls FORWARD-depth expansion; their exact physical angle/tilt assignment remains future tuning. Among active pairs, `M(2,2)` uniquely adds zero range. Every other `M(a,b)`, where `a,b ∈ {1,2,3}`, expands at least one of those dimensions; when both values differ from `2`, both dimensions expand. Metal never adds VERTICAL expansion. Off-center Metal is therefore a hunting/interception tool, not the harmonic solution. Exact curves and meter values remain tuning against moving targets.
- **METAL ROUNDING = PRESENTATION ONLY.** `M(2,2)` produces maximum fillet/edge rounding; moving either DOF toward `1` or `3` increases coverage on its dimension while strongly reducing its fillet contribution. The compact harmonic cage may look almost spherical/rounded-cuboid, but gameplay containment remains the nominal EARTH/WOOD/FIRE box-derived extent. Presentation fillet, bow and skin never enlarge detection authority; exact radii and combination curves remain tuning.
- **WATER = FREQUENCY / PHASE PRESENTATION AND SYNCHRONIZATION.** Water does not widen containment. Active angle maps `1 → GREEN`, `2 → BLUE`, `3 → VIOLET/PURPLE`; active tilt maps `1 → very dark`, `2 → medium luminance`, `3 → very bright`. `0` is OFF. Brightness is a Water frequency-state presentation dimension, not generic power. Exact colors and luminance values remain tuning.

The full harmonic array is `222 / M(2,2) / W(2,2)`: a balanced square core, zero added Metal range, maximum presentation rounding, and medium BLUE Water frequency. Its field is compact and box-derived but reads almost spherical/rounded-cuboid, with a subtle coherent BLUE pulse. It is neither maximum range, maximum field size nor maximum generic power. Pulse frequency, opacity/emission modulation and deformation amplitude remain tuning. The former broad-`M22` model and linear power-color ramp ending in WHITE are **SUPERSEDED**.

Installed Water plus the full harmonic array derives future **WATER SYNC LOCK** in the advanced Resonator domain. Lock plus the Haiku Cosmos canonical target anchor currently inside the active field derives **WATER SYNC CONTACT**. Contact strongly damps both angular and radial late motion without requiring a complete stop; the Large Glyph Actor remains physical motion owner. Configuration alone has no global slowdown.

Final Haiku Cosmos pull eligibility is a hard anti-bypass conjunction: learned Haiku/Water family + late context + `PULL_READY` + WATER SYNC CONTACT. Generic Target Acquisition remains Water-agnostic and may detect Haiku, show its sign, accumulate/retain/decay rings and reach `PULL_READY` before Water exists. Neither off-center Metal coverage nor retained readiness authorizes the final pull without current synchronized contact. Ether recovery must first enable Monkey's one exceptional Water rule intervention, after which Water becomes legally recoverable/installable and advanced Water control can exist. Ether remains special, not a sixth natural family. Exact intervention owner/effect/copy and Scenario point remain future.
...
| future advanced Resonator domain | Metal LATERAL/FORWARD expansion parameters and harmonic-center state; Water hue/luminance, Sync Lock and bounded Sync Contact truth | Large Glyph motion, acquisition stages/timers, target family |
...
Poza runtime i **NOT IMPLEMENTED** pozostają: Metal control/expansion/rounding response, Water control/hue/luminance/pulse, Ether-triggered Monkey exception, Water Sync Lock/Contact, Haiku slowdown and final Water-specific anti-bypass eligibility, future audio/finale APIs oraz Scenario point IDs after the existing `5.10` boundary. Early/late `PULL_READY` policy and late angular/radial motion are **IMPLEMENTED CURRENT**. Exact advanced mathematics, expansion, fillets, colors, slowdown, transitions and physical control remain tuning/future.
