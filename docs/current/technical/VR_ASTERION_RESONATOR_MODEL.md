...
## 8a. Late motion and Metal — CURRENT / IMPLEMENTED; Water — FUTURE
...
Metal implements `M(angle, tilt)` as two independently committed transient DOFs with levels `0/1/2/3`, physical detents `0°/13°/23°/36°`, dominant-axis gesture arbitration and composed physical sector motion. The provisional mapping is `angle → LATERAL`, `tilt → FORWARD`. Angle `1/3` adds `8 m` per lateral side and `0/2` adds none; tilt `1/3` adds nominal `10 m` depth under the global `10–130 m` clamp and `0/2` adds none. `M22` therefore adds zero gameplay range.
...
Exact gesture axes/signs, dominance margin, mapping, distances, composed-motion comfort and rounding readability are **TUNING / HARDWARE QA**. Hardware smoke confirms Metal tuning completion after the flat Shell identity fix and working acquisition after Metal installation, not final feel.
