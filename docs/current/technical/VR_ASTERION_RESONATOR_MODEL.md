...
The volumetric sector acquisition beam, bounded Platform Energy extension, discovery Guidance through first Resonator, and Scenario semantic join `4.80 → 5.10` are implemented. The runtime Field Presentation implements the superseded rounded cage; the revised side-aperture geometry and sector-layout forward alignment remain future implementation targets. Supported-object response, target scoring, Field audio, METAL/WATER contribution, and the later finale also remain future.
...
The bounded read-only level/angle snapshot and exactly-once semantic `DETENT_COMMITTED` subscription seam are implemented. The sector acquisition beam is implemented as described below. Spark VFX, target selection/scoring and response beyond current acquisition, Field/lensing presentation, Field audio and METAL/WATER motion or Field contribution are not implemented. R4's Field Actor and immutable analytic descriptor runtime are implemented from installed Rune truth and R2B committed levels; transient angles do not contribute.
