...
The volumetric sector acquisition beam, bounded Platform Energy extension, discovery Guidance through first Resonator, and Scenario semantic join `4.80 → 5.10` are implemented. The runtime Field Presentation implements the superseded rounded cage; the revised side-aperture geometry and sector-layout forward alignment remain future implementation targets. Physical target containment, resonance acquisition, sign/ring response, Astrolabium eligibility integration, Field audio, METAL/WATER contribution, and the later finale also remain future.
...
The lightweight 16-corner cage, translucent skin, curved skeleton, rounded fillets, and configuration morphing remain approved. Nominal aperture coordinates now create the primary shape; wall bow is secondary presentation tuning. Runtime still resolves the superseded fixed `S0–S3` origin-based geometry, mounts presentation under fixtures, and lacks intentional FIRE/entryDirection layout alignment. These are explicit superseding-decision implementation gaps, not claims of runtime alignment. The approved supported-object response is its family-colored Proto-Astro sign plus up to three thin resonance rings; no generic green halo or scoring mechanic belongs to the CURRENT target.
...
The sign and up to three thin target-centered rings use the target's Proto-Astro family color. The sign faces the current player head and retains approximately constant apparent size across depth. Exact angular/world sizing, clamps, ring dimensions, line thickness, spacing, pulse and fade curves remain **TUNING**. No renderer, shader, billboard, actor, registry, or runtime API is frozen.

Scenario may interpret discovery and gate crystal acquisition, but it owns neither containment nor resonance truth. Future containment owns the inside/outside decision; future resonance acquisition owns per-target stages, timers, decay, sign memory, and `PULL_READY`; Field Presentation is read-only.
...
| Resonator Field Domain / actor | authoritative active-field state, nominal field shape, read-only descriptor | physical MotionRoot, containment, resonance memory, attraction, Scenario meaning |
| future containment | canonical-anchor inclusion in the nominal fully active field | presentation geometry, resonance memory, attraction |
| future resonance acquisition | per-target completed stages, acquisition/decay timing, sign memory, `PULL_READY` | field geometry, attraction/pull, Scenario meaning |
...
Poza zakresem i nadal niezamrożone są: future target-response API/actors, shadery, parametry prezentacji/VFX/audio i Scenario point IDs after the existing `5.10` boundary. Semantyczne osie, poziomy i target detenty rdzenia, descriptor i język wizualny są CURRENT w sub-modelu pola.
...
The bounded read-only level/angle snapshot and exactly-once semantic `DETENT_COMMITTED` subscription seam are implemented. The sector acquisition beam is implemented as described below. Spark VFX, physical target containment/resonance and response beyond current acquisition, Field/lensing presentation, Field audio and METAL/WATER motion or Field contribution are not implemented. R4's Field Actor and immutable analytic descriptor runtime are implemented from installed Rune truth and R2B committed levels; transient angles do not contribute.
