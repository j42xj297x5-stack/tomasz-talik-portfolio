...
Audio is a shared, non-blocking subsystem described by `AUDIO_RUNTIME_MODEL.md`. `main` mounts one master/mute capsule outside `#app` from the language screen onward and delegates eligible button effects in both modes. Web Audio is unlocked only by interaction; Classic 2D never starts intro or ambient audio. Selecting Experience 3D only unlocks and prepares audio. After the loader is removed, the one-shot start sound and text intro begin together; ambient 01 enters five seconds later, and actual atmosphere levels map 0–1 to ambient 01 and levels 2–5 to ambient 02–05 through five-second equal-power transitions. Scene tuning exposes session-only ambient/effects bus gains under `?debug`; these values are deliberately absent from schema-version-1 scene import/export.
...
9. attempt the one-shot start sound and simultaneously play the localized opening intro, without making either wait for audio loading;
