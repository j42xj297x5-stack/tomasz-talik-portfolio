...
`syncTierOneWorldState()` and `syncAmbientSequence()` remain Runtime composition functions and have not moved into Scenario. Mainline after `2.40`, arbitrary Director start, hydration, reconstruction-backed checkpoints and durable save remain **NOT IMPLEMENTED**.
