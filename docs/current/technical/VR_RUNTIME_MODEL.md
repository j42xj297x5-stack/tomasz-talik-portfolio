...
The scene also contains a prototype progress floor at the world origin. It assembles a full circle from five unshifted sectors rotated at 72-degree intervals and uses four authored sectors: Creative AI / FIRE (`floor_creative.glb`), Ethics / EARTH (`floor_ethic.glb`), Haiku Cosmos / WATER (`floor_haiku_cosmos.glb`) and DIG Engine / METAL (`floor_dig_engine.glb`). DIG Engine occupies the `spotify-digger` rotation-zero slot and exposes four METAL panels; Haiku Cosmos exposes five WATER panels; Creative AI and Ethics expose three panels each. AI Guide / WOOD is the only remaining placeholder: it temporarily uses a separate Creative FIRE clone with three panels because a proper WOOD sector does not yet exist.

Visual activation is mapped by the `glyphId + order` pair and temporarily pulses, then steadily illuminates only the matching panel in that page's own sector. All four DIG Engine cards map in order to `VR_PROGRESS_CARD_METAL_01` through `VR_PROGRESS_CARD_METAL_04`; the former limitation affecting its fourth card has been removed. Activations accumulate independently and remain only in the prepared page runtime, including across XR session exit/re-entry. There is still no central `VrProgressionController` and no durable highlight state.
