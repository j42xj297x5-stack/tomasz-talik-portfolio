...
Every shell currently remains exclusively in `orbiting` state. It carries the future-facing `attractorTarget`, `attractorType = 'shell'`, unique `attractorId`, and `shellState = 'orbiting'` metadata, but Astro targeting and shell pull are not implemented. Session reset restores deterministic phases and then derives visibility again from `progressionController.isTierComplete(1)`, so prepared-runtime progression survives XR exit and re-entry.
...
Sector-background progression and its soft boundary, a central progression core, later world transitions, assembly orb, small glyphs, floor tilting and local-plane locomotion, antenna, runes, final radar, completion sequence, durable save, full-game reset and the full capabilities system are not implemented. Astro object targeting, shell gravity pull, capture, and held-world-object state are also absent. The current five-sector floor, its five global tier rings, the minimal card/tier controller, the tier-1 orbiting shell field, and the visual-only Astro runtime are the bounded implementations described above.
