...
Both controllers use one configured maximum interaction range of `2.3 m` for the visible pointer, active-glyph raycasts, available-crystal targeting/grab and the upper bound of reliquary button interactions. Trigger state never changes that maximum. The spatial pointer is a low-segment transparent mesh directed along local `-Z`: a `0.010 m`-diameter cylindrical shaft ends in a short tapered tip, with depth writes disabled.
