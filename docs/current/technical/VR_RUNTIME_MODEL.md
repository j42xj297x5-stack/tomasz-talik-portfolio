...
The scene contains a visually complete progress floor at the world origin. It assembles a full circle from five unshifted, authored sectors rotated at 72-degree intervals: Creative AI / FIRE (`floor_creative.glb`, three panels), Ethics / EARTH (`floor_ethic.glb`, three panels), AI Guide / WOOD (`floor_ai_guide.glb`, three panels), DIG Engine / METAL (`floor_dig_engine.glb`, four panels), and Haiku Cosmos / WATER (`floor_haiku_cosmos.glb`, five panels). DIG Engine occupies the `spotify-digger` rotation-zero slot. The floor no longer uses placeholder sectors. Each sector base is invisible, while its ornament and progression panels remain visible.

Visual activation is mapped by the `glyphId + order` pair and temporarily pulses, then steadily illuminates only the matching panel in that page's own sector. The complete ranges are FIRE 01–03, EARTH 01–03, WOOD 01–03, METAL 01–04 and WATER 01–05. Activations accumulate independently and remain only in the prepared page runtime, including across XR session exit/re-entry. There is still no central `VrProgressionController`, no durable highlight state, and no floor-tilting implementation.
