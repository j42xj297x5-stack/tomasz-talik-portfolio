...
With physically owned Astro equipped, right squeeze/grab above `0.1` shows the selected band's `VrAttractorScanCone` on controller-local `-Z`, even when that band currently has no legal target. Its length is `3R`, half-angle `2.5°`, current color `0x78ff9c`, opacity pulse `0.035–0.065`, and pulse period `1.6 s`. In particular, the `SMALL_GLYPHS` beam can display before Scenario point `4.30`; actual Small Glyph acquisition, halo and pull still require the existing `MATERIALIZED` system boundary, glyph `FIELD` state, visibility and family eligibility until S2.
