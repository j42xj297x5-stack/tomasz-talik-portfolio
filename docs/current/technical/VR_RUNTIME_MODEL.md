...
`createVrCrystalCollection` clones only preloaded AssetManager scenes. It deterministically derives authored-model scale (0.22–0.28), yaw, slight tilt, and an irregular XZ layout from `page.id`, centers each model by bounds, and moves its lowest bound to floor Y=0. State is explicit: `available`, `pulling`, `held`, or `consumed`.
