...
Ordinary Large Glyph ray hover, the existing `VrTargetHalo`, and existing glyph PointLight feedback are available either through normal crystal eligibility or through live late `SPHERE_FAR + PULL_READY` eligibility. The latter is presentation-only: select/hold continuation and crystal completion remain gated by current portfolio progression, so Resonator readiness alone cannot create a crystal. The ordinary ray maximum remains `2.3 m`.
