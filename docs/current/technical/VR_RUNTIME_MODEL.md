...
Exactly one each of `shell-relic-1` through `shell-relic-6` is required. `VrAstroFurnaceProgressionController` owns these six identity-based binary material slots independently from portfolio progression. Commit remains strictly gated by the conjunction `CONSUMED + COMPLETE`; interruption before it clears transient content without progress. At `6/6` the material owner remains unchanged, while the separate production controller opens `READY`. The **IMPLEMENTED** `UTWÓRZ` ray region accepts only a closed, stable chamber, empty content slot and idle furnace, then starts the furnace-owned `ASTERION_CONSTRUCTION` cycle. Its single normalized 18-second process progress drives mechanics, physical materialization and the panel; there is no production-side clock.

The production Sphere is parented to authored `VR_FURNACE_CONTENT_ANCHOR` and remains physically inside the chamber throughout `BUILDING` and `AVAILABLE`. During `0–6 s` it is nearly invisible/energetic; during `6–15 s` its private runtime materials reverse shell extraction (`opacity 0 → authored`, white color/emissive and high emission → authored values), and `15–18 s` restores the final authored material. In parallel, the panel transitions from the complete six-patch input sphere to a cached, progressively revealed contour map derived once from the real Asterion GLB. Completion commits `AVAILABLE` while the chamber remains closed. Opening does not move the Sphere; only `AVAILABLE + OPEN + left NORMAL_HAND` enables ordinary-ray halo and squeeze claim. Claim restores authored material and equipment scale, commits page-runtime `EARNED`, and auto-equips through `createVrHandModeController`. Session interruption cancels `BUILDING` to `READY`, removes the transient Sphere and preserves `6/6`; `AVAILABLE` and `EARNED` retain their existing page-runtime reset behavior.
