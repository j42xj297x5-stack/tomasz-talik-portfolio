...
WHERE, BEYOND, FOLLOW pause and hints are local `STAY` reactions. Crossing exists only at `1.130`: the Intro actor owns `playerEnteredRing` and `monkeySettled`, reports `crossingComplete`, and Director advances only after that combined fact resolves completion. The implemented late spine is `5.10 → 5.15 → 5.20 → 5.30 → 5.40 → 5.50 → 5.60 → 5.70 → 5.80 → 6.10 → 6.20 → 6.30 → 100.10`. Existing `resonatorExists` joins `4.80 → 5.10` through `RESONATOR_READY`, with `CHECK_RESONATOR_JOIN` covering either event order. This observes rather than gates physical Resonator creation. Subsequent points orchestrate the controlled Water rejection, Ether intervention, five-elemental-Rune/full-Resonator boundary, final portfolio completion, farewell, world release, credits, brand slate and terminal XR exit; their subsystem mechanics remain owned by the dedicated domain models.

Scenario defines that graph, its accepted semantic events, capabilities, milestones, settled consequences and symbolic entry effects. Director alone owns progression position, transition legality and one-time point activation. `RuntimeExperience` dispatches accepted events and executes the resulting symbolic effects against composed actors/domain owners; it does not decide the graph or absorb physical domain truth. `experienceVr.js` composes these owners, including the late Rune/Resonator communication, Ether intervention, final Monkey farewell, final-world-release, final ambient, credits and terminal-session handlers required by the implemented tail.

`stateAt(X)` is reconstruction state implied by settled Scenario history strictly before `X`; it does not replay transient work at `X`. Canonical arbitrary activation is `restore baseline → stateAt(X) → hydrate owners → synchronize derived state → replace Director at X → activateCurrentPoint()`. `activate(X)` therefore places a fresh Director at `X` and applies that point's entry effects once. Ordinary live transitions append destination entry effects to the accepted transition effects, while repeated activation of the already activated point is a no-op. Forward live catch-up remains distinct from reconstruction and follows authored spine order. These rules preserve the existing sandbox-reconciliation and point-authoring contracts.
...
Canonical main-background ownership follows `Scenario semantic audio entry → Ambient Sequencer → VrAudioBridge → audioManager`. The implemented late Scenario also emits symbolic final-ambient effects through its composed final sequencer. Detailed cue mapping, timing and remaining audio gaps are intentionally not synchronized here and remain owned by [`VR_AUDIO_MODEL.md`](VR_AUDIO_MODEL.md). The independent Asterion Sphere DEVICE loops are unaffected.
...
Entry to Scenario point `2.10` begins the shared linear first-world reveal for `VrCelestialActor` (the authored Sun and procedural Star Field), the five natural Rune Stones, Shell visuals and Small Glyph visuals. All use the celestial `60 s` reveal duration and fade from zero to authored opacity together. The Sun remains fixed beyond the scene fill PointLight on the same center-to-light ray and faces the world center; the deterministic clustered Star Field remains fixed across the full resolved `STARS` volume and `4π`. Reset restores the presentations to hidden/zero opacity, while reconstruction after completed `2.10` silently hydrates them at stable authored opacity; actor-owned celestial fade-out exists but is not connected to the Scenario finale.
...
A read-only target response presents each detected target's family-colored Proto-Astro sign and up to three rings. Large Glyph Astrolabium selection consumes natural-family knowledge in ordinary stages and additionally consumes `PULL_READY` at `SPHERE_FAR`, during candidate legality, immediately before pull start and throughout active pull; it has no Scenario permission gate and does not consult ordinary crystal eligibility. Loss of required readiness uses existing cancellation/return choreography; Astrolabium does not compute containment or acquisition and does not reset/freeze Resonator state. Detailed geometry and timing remain canonical in [`VR_ASTERION_RESONATOR_FIELD_MODEL.md`](VR_ASTERION_RESONATOR_FIELD_MODEL.md). Scenario observes the relevant domain results along its authored route through `100.10`; Resonator mechanics, audio and hardware/perceptual QA remain outside this core progression contract.
...
`CAN_USE_GLYPHS` is the Intro boundary for ordinary Large Glyph ray interaction: it is absent before `2.10` and continuous from `2.10 — GLYPH_FREE_EXPLORE` through the rest of the normal experience. After that boundary, physical reach determines whether the ordinary ray can hit a glyph, while `getNextCrystalTier(node)` independently determines whether the reached glyph may produce the next portfolio crystal. This capability is neither Astrolabium Large Glyph family knowledge, Resonator `PULL_READY`, nor portfolio crystal eligibility, and Asterion ownership does not gate ordinary Large Glyph acquisition.

Ordinary Large Glyph ray permission remains present in `SPHERE_FAR`; the physical distance normally keeps those glyphs outside the `2.3 m` ordinary ray. After Resonator reaches `PULL_READY` and Astrolabium pulls a known glyph back into range, the ordinary ray can interact with it immediately, still subject to current portfolio eligibility. `PULL_READY` additionally permits late Astrolabium pull and presentation feedback, but cannot itself create a crystal.
...
The authored bridge is `2.30 durable 5/5 → FIRST_RING_COMPLETED → 2.40`. `createVrFirstRingFlow` owns the local ring presentation/audio seam; only its real completion emits `FIRST_RING_PRESENTATION_COMPLETED` and enters `3.10`. Shell visuals are already present from the shared `2.10` world reveal; the existing `3.10` presentation call is idempotent, and the field remains non-interactive until physical Astro claim completes at `3.80`. Small Glyph visual presence likewise begins at `2.10`. Physical Astro production `EARNED` independently makes the Small Glyph field ready and moves passive canonical glyphs from `HIDDEN` to `FIELD`; `4.30` only runs and completes its authored presentation beat without rewriting physical or transient glyph state.
...
Semantic input maps standard-gamepad button `4` to edge-triggered `toggleRightTool` on the right hand and `toggleLeftTool` on the left hand, button `1` to analog `grabAction` (squeeze) and button `0` to analog `primaryAction` (trigger). Right A toggles `NORMAL_HAND ↔ ASTRO_ATTRACTOR` only after the physical Astro production owner reaches `EARNED`; Scenario and the intro QA bypass do not grant physical ownership. Physical `EARNED` also makes all four bands selectable in order: `SHELLS`, `SMALL_GLYPHS`, `LARGE_GLYPHS`, `RUNESTONES`. Left X toggles `NORMAL_HAND ↔ ASTERION_SPHERE` only after production `EARNED` or when the independent `?asterionSphere` QA availability override is present. `AVAILABLE` alone never enables either production-owned tool.

### Scan and targeting

With physically owned Astro equipped, right squeeze/grab above `0.1` shows the selected band's `VrAttractorScanCone` on controller-local `-Z`, even when that band currently has no legal target. Its length is `3R`, half-angle `2.5°`, current color `0x78ff9c`, opacity pulse `0.035–0.065`, and pulse period `1.6 s`. In particular, the `SMALL_GLYPHS` beam can display before Scenario point `4.30`. Small Glyph acquisition, halo and pull require both physical `fieldReady` projected from Astrolabium `EARNED` and a candidate in visible `FIELD` state whose family is targetable from processed Shell truth. Small Glyph extraction permission is the conjunction of physical Astrolabium ownership, Furnace physical legality and Proto-Astro extraction legality. Scenario no longer carries operational capabilities for ordinary Astrolabium equip, band switching, natural-family scan, target, pull, or Small Glyph extraction.
...
Y opens with Y and uses `MAIN_MENU → SECTION_DETAIL`, `MAIN_MENU → TOOL_LIST → TOOL_DETAIL`, and `MAIN_MENU → KNOWLEDGE_LIST → KNOWLEDGE_DETAIL`, with left-stick selection and X activation. Physical Astro acquisition grants equip and band switching across all four bands immediately; domain/tuning conditions govern target families rather than band existence. In `MAIN_MENU`, Y closes the panel; section and tool-list detail return to main, while tool detail returns to the tool list. Furnace enters the persistent list at actual reveal without granting Furnace capability. Astro/Asterion entries remain acquisition-driven.

Communication-section synchronization: **CURRENT as of 2026-09-07**. The implemented ownership model comprises Intro authored communication, Early Experience Guidance, Reliquary context Guidance, Rune/Resonator Guidance, Tool Guidance, and a shared authored-communication actor plus Monkey dialogue arbitration. These actors observe Scenario and domain transitions; they do not own gameplay truth. Hydration, direct activation and reset establish baselines so live discovery one-shots are not replayed. Y knowledge separately projects `SKORUPY`, `KAMIENIE RUNICZNE`, `ZWORNIKI` and `SEKTOR`; baseline reset clears this session memory and no durable save exists. Literal copy belongs to [`EXPERIENCE_VR_PLAYER_COMMUNICATION_COPY.md`](../concept/EXPERIENCE_VR_PLAYER_COMMUNICATION_COPY.md).
...
Monkey dialogue, attention and playback use an explicit owner lease with strict priority `MANDATORY > ACQUISITION > OPTIONAL`. Only one lease is active. Owner-guarded operations cannot clear another actor's state; a strictly higher priority may preempt lower pre-playback `WAITING`/`ATTENTION`/`AUTO_DELAY` work, while started `PLAYBACK` is non-preemptible. Pending communication may be cancelled when its originating condition stops being relevant. The shared authored actor supports `WAITING`, `ATTENTION`, `AUTO_DELAY`, `PLAYBACK` and `COMPLETE`; successful completion releases the lease and returns to silent idle without opening the ordinary menu.

Required authored work uses the existing sound plus visual attention arcs and waits for a Monkey press. Semantic `AUTO_HINT` instead calls the sound-only cue, waits exactly `1.0 s` in `AUTO_DELAY`, and starts playback automatically without arcs or a press. The sound-only method invokes the same audio callback without changing attention ownership, `attentionPending`, arc visibility or opacity. Intro and shared authored speech exclusively own the Monkey surface through completion: ordinary `CO TERAZ?`, `JAK MI IDZIE?`, history and knowledge cannot open alongside it. This lease does not block locomotion, tools or world interaction.

The seven current `AUTO_HINT` keys are owned by three guidance areas:

- Early Experience: `hint.crystal.whatNow.soft` and `hint.crystal.grab.medium` use consecutive existing `30 s` pickup stages in `first-crystal-pickup`; `hint.reliquary.firstCrystal` uses the existing `60 s` post-reveal/unadvanced condition in `first-crystal-reliquary`.
- Reliquary context: `hint.reliquary.inserted` and `hint.reliquary.active` retain the Scenario route from each phase-local `15 s` timeout and share `reliquary-context`; entering active starts its timer from zero.
- Rune/Resonator: `hint.rune.noBinder.soft` runs after `5 s` unresolved and medium after another unresolved `5 s`; both share `rune-no-binder` and reset/withdraw when `getUnresolvedRuneBranchId()` becomes falsy.

After—and only after—successful automatic playback completion, these actors may publish their authoritative existing blocks into the Monkey knowledge resolver's generic session-local transient fallback store. Timer trigger, scheduling, cue, `AUTO_DELAY`, playback start and cancelled pre-playback work publish nothing. Entries are `NEW`, derive their ordinary question label from the final authored block and appear before, rather than replace, the existing ordinary `CO TERAZ?` current-guidance topic.

Escalation replacement is slot-local: pickup medium replaces unread pickup soft, Reliquary active replaces unread inserted, and noBinder medium replaces unread soft. Unrelated slots coexist in deterministic projection order without a global semantic rank or latest-wins policy. Gameplay owners independently withdraw obsolete slots; full deliberate knowledge playback also consumes an entry. Opening or selecting does not consume it. Withdrawal may remove future availability while an already-started deliberate reading finishes, and that completion cannot resurrect the entry.

Publication and withdrawal call `refreshKnowledge()` so an already-open ordinary menu or `CO TERAZ?` view redraws from current resolver state without close/reopen. Refresh neither opens/closes Monkey nor changes dialogue ownership or exposes ordinary UI during authored speech. Transient fallback memory is neither Scenario/gameplay truth, Player Y knowledge, card history, permanent knowledge nor persistence; canonical Monkey/session reset clears all slots with no durable reconstruction.

Automatic playback is not itself `AUTO_HINT`: progression reactions including `progression.threshold.crossed`, `progression.crystal.firstCreated`, `progression.card.first` and `progression.p3.firstSectorLock` create no transient fallback merely because they play automatically. Current acquisition teaching also remains distinct. `hint.glyphs.how.soft`, `hint.glyphs.how.strong`, `hint.protoAstro.tuning`, `hint.furnace.astroStart` and `hint.furnace.astroAvailable` retain their existing click-required/attention behavior and were not migrated.
...
Large Glyph is not a spherical layer. Its implemented actor stages are `RING_INITIAL = 8.5 m`, `RING_ELEVATED = 8.5 m + 4.0 m`, `RING_EXPANDED = 46 m` and `SPHERE_FAR = 80 m`. After Tier 3 five glyphs occupy deterministic full-sphere directions and move at `0.01 rad/s`. At `SPHERE_FAR`, each target uses its actor-owned black, light-insensitive materials until its authoritative Resonator acquisition state reaches `PULL_READY`; readiness restores that exact glyph's existing authored light-reactive materials, while live decay to two rings restores black. Earlier stages always retain authored materials. Scenario hydration does not persist Resonator readiness.
...
The composition includes the late runtime owners needed to execute those symbolic effects and emit their completion events. Detailed Resonator, Water, Rune, audio and finale presentation contracts remain in their dedicated documents. Outside this Scenario/runtime synchronization, Panels 3–4, durable full-game persistence and full-game reset remain unimplemented.

## Closed runtime reconciliation gaps

Recipe insertion family validation, recipe-change player-facing eject, debug `P5 → 4.80` and `P6 → 5.10`, early natural Rune Stone presentation, A9.5 handoff choreography, A9.6 hydration/reconstruction foundation, physical Resonator target response and the authored continuation through `100.10` are **IMPLEMENTED**. The debug aliases remain bounded QA entrypoints rather than alternate Scenario routes. Carried Rune Stone ↔ installed Rune Stone collision is superseded and is not a future target.
