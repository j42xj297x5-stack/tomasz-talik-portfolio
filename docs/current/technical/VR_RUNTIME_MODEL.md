...
The scene also contains a prototype progress floor at the world origin. It assembles a full circle from five unshifted sectors rotated at 72-degree intervals and uses three authored source types: Creative FIRE (`floor_creative.glb`), Ethics EARTH (`floor_ethic.glb`) and Haiku Cosmos WATER (`floor_haicu_cosmos.glb`). Creative AI occupies its target upper-left slot with three FIRE panels, Ethics its target lower-left slot with three EARTH panels, and Haiku Cosmos its lower-right slot with five WATER panels. AI Guide and DIG Engine retain separate Creative-model placeholders in their existing slots; each visualizes its first three pages with its own FIRE panels. DIG Engine's fourth page still appears in the portal but has no matching visual floor panel.

Visual activation is mapped by the `glyphId + order` pair and temporarily pulses, then steadily illuminates only the matching panel in that page's own sector. Activations accumulate independently and remain in the prepared floor runtime across XR session exit/re-entry. There is still no central `VrProgressionController`.
