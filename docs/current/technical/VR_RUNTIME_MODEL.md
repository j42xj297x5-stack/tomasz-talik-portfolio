...
The panel is a read-only consumer of process state/progress, angular speed, accumulated process angle and content state. Its procedural Canvas 2D monitor draws a pulsing wireframe silhouette of the inserted shell which fades through absorption, and a gently rotating six-segment wireframe preview of committed Asterion Sphere progress. IDLE communicates readiness without a progress bar or false `100%`; COMPLETE confirms that the essence was stored in furnace memory. While a process (or its short completion display) is active, canvas redraw defaults to `12 Hz`, normalized to `4–30 Hz`; otherwise updates remain event-driven.
