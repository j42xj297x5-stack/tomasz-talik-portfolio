...
The scene contains a visually complete five-sector progress-floor foundation at the world origin. It assembles a full circle from five unshifted, authored sectors rotated at 72-degree intervals: Creative AI / FIRE (`floor_creative.glb`, three panels), Ethics / EARTH (`floor_ethic.glb`, three panels), AI Guide / WOOD (`floor_ai_guide.glb`, three panels), DIG Engine / METAL (`floor_dig_engine.glb`, four panels), and Haiku Cosmos / WATER (`floor_haiku_cosmos.glb`, five panels). DIG Engine occupies the `spotify-digger` rotation-zero slot. All five sectors are authored implementations; each base is invisible while ornaments and progression panels remain visible. The complete subsystem contract is documented in [`VR_PROGRESS_FLOOR_MODEL.md`](VR_PROGRESS_FLOOR_MODEL.md).

Visual activation is mapped by the `glyphId + order` pair and produces an initial impulse followed by stable illumination only on the matching panel in that page's own sector. The complete ranges are FIRE 01–03, EARTH 01–03, WOOD 01–03, METAL 01–04 and WATER 01–05. Activations accumulate independently and remain only in the prepared page runtime, including across XR session exit/re-entry. This visual card mapping is not a completed threshold system: there is still no central `VrProgressionController`, global tiers or rings, durable highlight state, or floor-tilting implementation.
...
The approved roadmap's branch-bound crystals, Activate-time sequential page resolver, central progression controller and global rings, gameplay tiers, shells, assembly orb, hand tools, small glyphs, floor tilting, runes, final radar, completion sequence and durable save system are not implemented. The current five-sector floor is the bounded visual implementation described above.
