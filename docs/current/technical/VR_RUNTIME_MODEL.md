...
The composition root owns the small `VrAudioBridge` lifecycle. Optional VR audio requests cross its fail-soft boundary, which contains both synchronous exceptions and Promise rejections and reports them as `[vr-audio]` warnings. Calls are fire-and-forget side effects after gameplay transitions; they are never awaited by session, input, progression, state-machine or render-frame flows. The bridge delegates Web Audio ownership to the shared `audioManager`, stores no gameplay state and is disposed idempotently on `pagehide`. Five unity-gain VR buses (`SPACE`, `AMBIENT`, `DEVICE`, `WORLD`, `UI`) now feed the existing Master Volume/mute. The first UI package is active for player-panel open/close/internal navigation, monkey-panel open/close/internal clicks, and Astro Furnace panel open/deeper/back navigation. A second one-shot package is active for accepted reliquary insert and consume commit, idempotent tier completion, new monkey communication signals, and accepted Astro Furnace chamber open/close transition starts. Glyph acquisition process audio and mapped elemental completion one-shots are active as fail-soft side effects of the existing hold/crystal-creation flow. The process uses same-glyph one-second perceptual recovery, a 0.1-second resume ramp, a 0.2-second success/cancel fade, and manual renewal after natural source completion. Ambient sequencing, Astro Attractor audio and spatial audio remain **not implemented**.
...
The Astro Furnace is a separate subsystem backed by `public/glb/astral_stove.glb`, preloaded through `AssetManager`, placed opposite the portal under `VrPlatformFixturesRoot`, grounded from visible bounds, scaled to `3` and faced toward the configured start. Authored open/close animation owns `PIVOT_FURNACE_CHAMBER_Z`; runtime spin owns only `PIVOT_FURNACE_PROCESS_SPIN`, and the lid never spins.
...
Exactly one each of `shell-relic-1` through `shell-relic-6` is required. `VrAstroFurnaceProgressionController` owns these six identity-based binary material slots independently from portfolio progression. Commit remains strictly gated by the conjunction `CONSUMED + COMPLETE`; interruption before it clears transient content without progress. At `6/6` the panel shows a complete material/holographic reconstruction. It is **not** a physical Asterion Sphere and does not construct or materialize one.

## QA physical Asterion Sphere prototype

`?asterionSphere` enables the QA physical Asterion Sphere / Kula Asterionowa. It does not commit shells, does not alter `VrAstroFurnaceProgressionController`, does not fake `6/6`, and does not reinterpret the furnace panel preview as a production physical object. The runtime loads `public/glb/asterion_sphere.glb` through `AssetManager`; the hand-mode controller equips the cloned model on the resolved left `grip` only in left `ASTERION_SPHERE` and unequips it in left `NORMAL_HAND`.
...
Implemented: runtime/session lifecycle, platform-relative hierarchy, player passenger root, platform fixtures under `VrTiltableFloorRoot`, platform-local locomotion with radial boundary, glyph/crystal/reliquary progression, floor panels/rings, Tier-1 shell activation, semantic Astro input, independent right Astro Attractor and left Asterion Sphere hand modes, analytic scan targeting, pull/cancel/return, explicit left-ray handoff when left is free, placement and ordinary-ray re-grab, Astro Furnace asset/placement, open/activate/option/content interactions, Asterion panel, six-type furnace progression, physical shell insertion/retrieval/absorption, COMPLETE-gated commit, QA physical Asterion Sphere, PREVIEW/COMMAND/CURRENT gyro contract and heavy angular platform drive.

Not implemented: production physical Asterion Sphere construction, production equipment gating, `UTWÓRZ`, production 6/6 materialization, small glyph progression, B band selection/Astro bands, antenna, rune processing, final radar/finale, durable persistence, full-game reset and VR gameplay audio beyond the bounded one-shot packages. The fail-soft VR audio boundary and glyph-acquisition lifecycle are implemented independently of that future content.
