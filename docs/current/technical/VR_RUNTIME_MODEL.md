...
The composition root also owns the small `VrAudioBridge` lifecycle. Future optional VR audio requests must cross its `runOptional(operation, request)` boundary, which contains both synchronous exceptions and Promise rejections and reports them as `[vr-audio]` warnings. Calls are fire-and-forget side effects after gameplay transitions; they are never awaited by session, input, progression, state-machine or render-frame flows. The bridge delegates Web Audio ownership to the shared `audioManager`, stores no gameplay state and is disposed idempotently on `pagehide`. There are currently no VR playback calls or VR gameplay audio assets: the fail-soft boundary is implemented, while VR gameplay audio content remains **not implemented**.
...
Not implemented: production physical Asterion Sphere construction, production equipment gating, `UTWÓRZ`, production 6/6 materialization, small glyph progression, B band selection/Astro bands, antenna, rune processing, final radar/finale, durable persistence, full-game reset and VR gameplay audio content. The fail-soft VR audio boundary is implemented independently of that future content.
