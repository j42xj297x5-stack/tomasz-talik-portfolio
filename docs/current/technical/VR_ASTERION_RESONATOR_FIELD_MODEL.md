...
Water dual-DOF physical control, descriptor state, hue mapping, monotonic skin/skeleton luminance, additive halo and smooth presentation morphing are **IMPLEMENTED**. Genuine **FUTURE / NOT IMPLEMENTED** scope includes Water-specific breathing pulse, Water Sync Contact gameplay, harmonic recognition/response, Haiku damping, actual Glyph-family filtering by Water frequency and the Water-specific final anti-bypass gate, plus later Scenario/finale systems. The descriptor's derived `waterSyncLock` flag is descriptive state only and does not constitute those future gameplay systems. The late-only generic `PULL_READY` policy and Large Glyph angular plus `20–110 m` radial motion are implemented; their numeric tuning and hardware QA remain outstanding.
...
Water-specific breathing pulse, Sync Contact gameplay, harmonic synchronization/response, Haiku damping, Water-specific pull eligibility, Resonator field/target audio, later supported target registrations and later finale systems remain **FUTURE / NOT IMPLEMENTED**.
