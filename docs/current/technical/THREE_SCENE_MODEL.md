...
- Creative AI alone has an opt-in plaque pilot. Its `/glb/plaque_creative_ai.glb` asset is staged in `deferredWarm` and `src/scene/plaqueTransition.js` obtains it only through `AssetManager` (or its on-demand `loadAsset` cache path), then keeps one reusable cloned instance. The controller normalizes its Box3 bounds, uses independent clone materials, disables raycasting, and cross-fades it with the cloned-material glyph while a short local warm transition runs.
- The guarded Creative AI sequence is `focusing` → `revealingPlaque` → `dollyIn` → `panelOpen` → `dollyOut` → `restoringGlyph` → `returning` → `idle`. The overlay opens only after the plaque reveal and camera dolly; closing keeps the selected node reference, reverses the dolly and cross-fade (at roughly half duration), then uses the existing return-home and 1500 ms pointer handoff contract.
- `cameraRig.dollyToPlaque(...)` changes only camera radius while retaining the canonical pivot and cinematic yaw. It derives a cover distance from plaque world bounds, vertical/horizontal FOV and aspect, clamps against the near plane, and stores the exact focus pose used by `dollyOut(...)`; resize does not recompute an open-panel pose.
