...
- Atmosphere progression is cumulative and has one canonical order: **1. shells → 2. small glyphs → 3. stars → 4. animated stones → 5. galaxies and the Milky Way**. Level 0 shows only the base world—monkey, main glyphs, sun, and moon—and hides all five progressive atmosphere layers. Every earlier layer remains visible after a later level unlocks. Each of the five layers reveals over 10 seconds by default; manual transition-time settings remain supported. Galaxy sprites and the Milky Way share the same `galaxies` multiplier at level 5.
...
Stone positions use spherical Fibonacci slots generated from the actual instance count, giving the complete spherical layer an even directional distribution. Each rebuild applies one random global rotation to the slot set and independently samples every slot's radius with the existing volume-uniform shell distribution, preserving an organic result. Available model sources are shuffled once per rebuild and then assigned cyclically, so type counts differ by at most one and adjacent slots use different types when more than one source is available. When the instance count is lower than the available source count, distinct shuffled sources are prioritized before any type repeats; this also makes reduced mobile counts and future `count` changes self-adjusting.
