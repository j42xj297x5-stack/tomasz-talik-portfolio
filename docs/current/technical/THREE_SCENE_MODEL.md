...
- Creative AI (`/glb/plaque_creative_ai.glb`) and AI Guide (`/glb/plaque_ai_guide.glb`) opt into the same plaque sequence through their content metadata. The existing data-driven `deferredWarm` manifest maps them to `plaque-creative-ai` and `plaque-ai-guide` without per-asset manifest entries. `src/scene/plaqueTransition.js` obtains a requested plaque only through `AssetManager` (or its on-demand `loadAsset` cache path).
