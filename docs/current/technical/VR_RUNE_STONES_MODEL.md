...
- **Canonical authored Scenario/runtime progression boundary:** `5.30`; `FOURTH_RUNE_INSTALLED → Monkey Ether reveal → CAN_TUNE_ETHER_RUNE` is implemented/authored after the `4.80 → 5.10` join.
...
Obowiązkowy beat Małpy po `FOURTH_RUNE_INSTALLED` jest zaimplementowany przez Scenario `5.20`; jego ukończenie prowadzi do `5.30` i semantycznego `CAN_TUNE_ETHER_RUNE`. Jest to wyłącznie permission dla następnego etapu. Fizyczne strojenie Eteru nie jest jeszcze zaimplementowane.
...
Authored Scenario contains the bounded semantic join `4.80 → 5.10` and continuation `5.10 → 5.20 → 5.30 → 100.10`. The live Rune snapshot crossing from fewer than four to exactly four installed natural families projects `FOURTH_RUNE_INSTALLED`; `5.20` owns the mandatory Monkey Ether reveal, and its completion grants `CAN_TUNE_ETHER_RUNE` at stable `5.30`. Hydration restores settled Rune truth without replaying transient communication. Physical Ether tuning, transport/capture, Water override and later finale remain future.
...
R3 implementuje runtime Zwornika, a R3b implementuje `RUNE_BINDER_REVEAL`: live materialization, pooled sector-local lightning, final pulse, trwały `DOCKED`/`BOUND`, presentation-only scale/offset i reconstruction bez replayu. Nadal nie implementuje Rune install energy VFX, detent spark/audio i grip beam; sector-drive audio jest wdrożone osobno, Field Actor ani field lensing. A9.6 nie implementuje Water override trigger ani fizycznego Ether/final Water flow; installed spatial audio jest wdrożone przez osobną projekcję. R4 Resonator core, Guidance through first Resonator and semantic Ether unlock through `5.30` are implemented by their respective owners.

## 14. Remaining target

**Carried Rune Stone ↔ installed Rune Stone collision = SUPERSEDED / REMOVED FROM TARGET.** Physical Ether tuning/transport/capture, Water readiness override and final Water flow after the authored `5.30` boundary remain deferred. Zwornik spin jest usunięty z target canon.
...
AUTHORED SCENARIO BOUNDARY: 5.30
