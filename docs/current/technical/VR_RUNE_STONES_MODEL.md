...
**VISIBILITY ≠ TARGETABILITY ≠ INSTALLATION READINESS.** Wizualna obecność, tuning i targetability są niezależnymi prawami. Po fizycznym pozyskaniu Astrolabium pasmo `RUNESTONES` jest selectable nawet przy pustym `tunedRuneFamilies` i pustym zbiorze legalnych celów; to **CURRENT / IMPLEMENTED (S1)**. Dopiero późniejszy tuning nadaje naturalnej rodzinie prawo targetowania, a platform installation readiness pozostaje trzecim, osobnym prawem gameplayowym opisanym poniżej.
...
**Canonical live installation audio:** `installAudioLeadSeconds = 1.0`. Branch-specific `creating_01–05.mp3` rozpoczyna się dokładnie raz, gdy pozostały czas finalnej fazy `DESCENT` po raz pierwszy wynosi `<= 1.0 s`; jeśli przyszły tuning skróci całe `DESCENT` do `<= 1.0 s`, one-shot rozpoczyna się przy wejściu w `DESCENT`. Jest to wyłącznie anticipacja prezentacyjna: nie wolno przenosić triggera do `APPROACH`, `BRIDGE_OPEN` ani attractor handoff, a stan `INSTALLED`, bridge `BOUND`, progression `commitInstalledFamily()` i installed spatial emitter pozostają legalne dopiero po fizycznym settlement. Hydration/reconstruction nie odtwarza `creating_*`.

**Accepted docking audio — IMPLEMENTED:** successful natural `beginSocketCapture(branchId)` emits one observer-only docking-start event. It immediately plays a 13 m WORLD spatial one-shot from the moving physical stone (WOOD/WATER → `stone_landing_01`, FIRE → `02`, EARTH → `03`, METAL → `04`), then exactly `3.0 s` later reads the same stone's current transform for `zwornik_dokowanie.mp3`. This does not replace the final-DESCENT `creating_*` cue. Reset/reconstruction is silent and cancels pending follow-ups. SPECIAL Ether never uses either docking sound.
...
Pair-specific audio identity jest fizyczna: FIRE / `stone_01` → `noise_laud_loop_04.mp3`, METAL / `stone_02` → `05`, EARTH / `stone_03` → `06`, WOOD / `stone_04` → `07`, WATER / `stone_05` → `08`. Te same assets obsługują dwa osobne wdrożone lifecycle: Astro Attractor od zaakceptowanego physical pull do cancel/handoff oraz persistent installed spatial loop. Hover/targeting nie uruchamia audio, a wcześniejsza cicha emisja przez `FREE` i `CARRIED_ORBIT` jest **SUPERSEDED**. Persistent source wymaga jednocześnie physical `INSTALLED` i installed-family progression truth; nie istnieje w `FREE`, `LOCKED_BY_ASTRO`, `CARRIED_ORBIT` ani `SOCKET_CAPTURE`. Stan kamienia rozstrzyga **czy** source istnieje, lecz jego pozycję rozstrzyga stabilny Progress Floor sector anchor: sector-local `+Z`, 8.0 m od środka, sibling `SectorMotionRoot`, więc dziedziczy globalny transform/gyro, authored layout yaw i statyczny sector yaw, ale nie detent pitch/tilt; nie śledzi stone root, installation anchor ani bridge center. Shared listener jest aktualizowany raz na VR frame przez composition z `getXrHeadWorldPose(...)`, a Rune projection aktualizuje wyłącznie emitery. Current `DEVICE`/HRTF/linear source ma `maxDistanceMeters = 4.0`, `refDistanceMeters = 0.25`, `rolloffFactor = 1` i `loop = true`; przy `>= 4.0 m` gain wynosi zero. `platformRadiusMeters = 8.0` oznacza wyłącznie promień audio anchor, a nie gameplay distance. Pełny Web Audio i reconstruction/reset contract posiada [`VR_AUDIO_MODEL.md`](VR_AUDIO_MODEL.md).
...
Ether Rune Stone nie należy do pięciu początkowych naturalnych kamieni platformy. Jego presentation pojawia się przez `REVEAL_ETHER_RUNE` przy wejściu w `5.40`. Po special tuning gracz targetuje i przyciąga Ether wspólną mechaniką Astrolabium aż do istniejącego handoff radius. Dedykowany SPECIAL handoff przenosi ownership z Astrolabium do `VrEtherMonkeyCaptureInteraction`, przechodzi `CARRIED_ORBIT → MONKEY_CAPTURE` i interpoluje przez 1,5 s do live platform-local `VrEtherMonkeyHoverAnchor` na `(0, 3, 0)`. Po ukończeniu ten sam widoczny root, z zachowaną authored skalą, hierarchią GLB i animacją, zostaje platform-bound nad Małpą jako nietargetowalny `CAPTURED`; hydration odtwarza ten settled stan bez replayu lotu. Nie używa naturalnego installation callbacku, Bindera, frame ani `installedRuneFamilies`. Its accepted Astro pull uses `noise_laud_loop_09.mp3` through the existing DEVICE Attractor lifecycle and handoff fade; it has no docking or persistent installed audio. Jedyny trwały commit ustawia monotoniczne `waterInstallationReadinessOverride: true`; live projection uruchamia istniejące Water `HIDDEN → ARRIVING → DOCKED`, a hydration synchronizuje bez replayu.

Stan `CAPTURED` automatycznie aktywuje platform-bound `VrEtherMonkeyPresentation`, także po cichej reconstruction bez historycznego callbacku. Actor prezentacyjny nie posiada ani nie reparentuje fizycznego rootu: nakłada wyłącznie lokalny idle względem `VrEtherMonkeyHoverAnchor`, zachowując authored skalę i animacje GLB. Ruch używa współdzielonego `placedObjectIdleMotion`: pion ma 0,20 m peak-to-peak w cyklu 4,8 s, poziomy orbit ma promień 0,06 m z 1,5 s smoothstep wejściem, a self-rotation prędkość 0,12 rad/s od settled local quaternion.

Ten sam actor posiada jeden nieinteraktywny, addytywny `Mesh` pod `VrPlatformFixturesRoot`: jedna współdzielona `BufferGeometry` zawiera sześć niezależnych rur (`18` odcinków ścieżki × `6` boków każda), których matematyczne source circle jest na wysokości 1,10 m i ma promień 0,25 m. Centerlines tworzą półobrotową (`π`) spiralną klatkę z bulge 0,80 m, endpoint-safe winding i fazami pulse rozstawionymi co 60°; każdy endpoint jest w każdej aktywnej klatce wyliczany z bieżącej world pozycji kamienia i przeliczany do platform-local. Circle i bounding sphere pozostają wyłącznie konstrukcjami matematycznymi — nie istnieją jako widoczne meshe — a prezentacja nie tworzy światła, kolizji, targetu, audio ani gameplay truth. Wszystkie liczby kształtu, pulsu i idle są **TUNING**; jedyną prawdą aktywacji pozostaje fizyczny stan `CAPTURED`. Reset usuwa wyłącznie presentation-owned pose offsets przed resetem fizycznego actora, a dispose zwalnia tylko własną geometrię i materiał.

Podczas finalnego world release istniejący owner przejmuje ten sam `VrEtherMonkeyHoverAnchor` wyłącznie wtedy, gdy fizyczny Eter pozostaje `CAPTURED` i jego root nadal jest dzieckiem anchora. Actor zapisuje pełny local transform i pierwotnego parenta, pobiera startową world position, po czym `scene.attach()` odłącza anchor od poruszającej się platformy bez zmiany widocznej world pose. Lot używa dokładnie wspólnego finale `motionFactor(time)` i `RELEASE_DISTANCE`: world target ma postać `releaseStartWorldPosition + (0, RELEASE_DISTANCE * motionFactor(time), 0)`, czyli +10 m po 5 s i +30 m po 10 s. Fizyczny root pozostaje dzieckiem anchora, a jego mixer GLB oraz istniejący idle bob/orbit/self-rotation są aktualizowane także w zablokowanej gałęzi finału.

Sześć tendrili zachowuje winding, pulse, vertex-color gradient i platformowy matematyczny source, podczas gdy endpoint nadal śledzi bieżącą world pozycję odlatującego kamienia. Przez pięć sekund fazy `RELEASE` niezależny mnożnik opacity wynosi `1 - clamp01(time / 5)`; geometria jest aktualizowana do osiągnięcia zera, a dopiero wtedy mesh zostaje ukryty. Reset final-release ownera przywraca pierwotnego parenta i kompletny local transform anchora, natomiast reset presentation ownera czyści release mode, przywraca opacity `1`, baseline visibility oraz zeruje własny czas; zasoby modelu i tendrili nadal są zwalniane wyłącznie przez ich dotychczasowych ownerów. Fizyczne `CAPTURED` i Water readiness nie są przez ten takeover zmieniane.
...
**Presence/visibility nie oznacza targetability.** Od `2.10` pięć naturalnych Rune Stones może być widoczne przy `tuned = false` i `targetability = false`; po fizycznym `EARNED` band `RUNESTONES` jest mimo to selectable, lecz natural targetability nadal równa się `tunedRuneFamilies` przez istniejącą A7 projection. Hidden presentation dodatkowo blokuje physical candidate legality. Ether nie jest szóstym naturalnym recordem, nie jest ujawniany w `2.10` ani dodawany do natural targetability; pozostaje SPECIAL dla przyszłego Ether/Monkey flow.
