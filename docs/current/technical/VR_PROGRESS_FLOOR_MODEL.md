...
Progress Floor remains the bounded physical projection for R2B through `setSectorMotion`, `getSectorMotionTransform` and `resetSectorMotion`; mutable sector actors and MotionRoots remain private. Defensive `getSectorControlFrame(glyphId)` exposes only the stable sector frame needed to interpret hand intent, while `getSectorMotionBounds(glyphId)` returns cloned flat-state presentation min/max in canonical MotionRoot-local space. EARTH rotates around `[sin(36°), 0, cos(36°)]` and WOOD around its mirror `[-sin(36°), 0, cos(36°)]`: both axes are outer radial wedge edges through origin. Opposite physical signs lower both inner edges toward FIRE while MotionRoot.position stays exactly `(0,0,0)` at every level. FIRE (`+X`) retains its inner radial `minZ` bounds pivot and `P - R(P)` translation. LEVEL 0 remains exact identity and the complete MotionRoot subtree remains rigid.
...
Separately, each Sector Actor owns a flat invisible `VrAsterionSectorTargetSurface:<glyphId>` copied from the authored reference BASE with shared geometry and its flat transform derived relative to ActorRoot. Its owned transparent, color-write-disabled DoubleSide material keeps it raycastable without presentation. The surface is an ActorRoot child above MotionRoot, so it follows `VrTiltableFloorRoot` but never local sector tilt. `raycastAsterionSectorTarget(raycaster)` returns only a defensive nearest-hit descriptor; acquisition raycasts only these buttons, then independently applies Rune-powered truth. Reset leaves them constructionally stable and dispose releases only the owned target material, never shared authored geometry. The beam endpoint remains the moving panel-3 anchor.

The presentation-only whole-sector overlay reuses authored geometry with an owned additive material and does not overwrite progression emissive state. It is hidden at zero, cleared by reset/dispose, and its owned material is disposed with the sector. Acquisition lightning and `FLOOR_DRIVE` are implemented through the shared Platform Energy actor. Detent sparks, drive/detent audio, `RUNE_INSTALL` VFX/audio, Field/lensing presentation, target response beyond the implemented acquisition contract and METAL/WATER contribution remain **NOT IMPLEMENTED**.


## Platform energy extension — IMPLEMENTED

A presentation-only projection now adds existing-progress `SECTOR_ACQUISITION` lightning and actual-angle-change `FLOOR_DRIVE` lightning without changing acquisition, R2B, Field, or progression truth. Acquisition stops at `LOCKED`; stationary `DRIVING`/`DETENT_HOLD` produces no drive energy, while physically changing `SETTLING` angles do. The single shared bounded pool emits sector surface bolts and fail-soft Binder feeds to the authored `BRIDGE_STONE_CAPTURE` inside the scaled, radially offset bridge presentation subtree. World-to-sector-local conversion preserves the true endpoint through the same downward MotionRoot hinge inheritance. Bounded one-generation branches are implemented; independent multilayer bolt shells remain future. `RUNE_INSTALL`, detent sparks, motion/detent audio, Field/lensing, target response beyond acquisition, and Metal/Water motion/contribution remain not implemented. Hardware QA remains outstanding.
