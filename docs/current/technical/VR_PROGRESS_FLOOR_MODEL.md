...
Progress Floor remains the bounded physical projection for R2B through `setSectorMotion`, `getSectorMotionTransform` and `resetSectorMotion`; mutable sector actors and MotionRoots remain private. Defensive `getSectorControlFrame(glyphId)` exposes only the stable sector frame needed to interpret hand intent, while `getSectorMotionBounds(glyphId)` returns cloned flat-state presentation min/max in canonical MotionRoot-local space. EARTH (`+Z` rotation) uses the `maxX` side hinge, WOOD (`-Z`) the mirrored `minX` hinge, and FIRE (`+X`) the inner radial `minZ` hinge. Hinge Y is the flat presentation ceiling `bounds.max.y`; pivot translation is `P - R(P)`, so the complete MotionRoot subtree moves rigidly and no presentation geometry gains a new upward excursion above that ceiling. LEVEL 0 remains exact identity. The physical direction supersedes the earlier runtime mirror across the flat platform surface.
