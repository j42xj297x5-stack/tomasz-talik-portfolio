...
- M2.2A is **IMPLEMENTED**: ordinary LIVE transitions author `kind: STAY | COMPLETE | EXPLICIT`. `STAY` retains the point, `COMPLETE` obtains the authored successor from Spine, and `EXPLICIT` requires a concrete local/exit/loop target. M2.2C adds the deliberately narrow `COMPLETE_IF` join described below. Director publishes the resolved `transitionKind`; Runtime only forwards the accepted change and executes effects. Missing/unknown kinds and invalid kind/target combinations fail Scenario validation.
...
- M2.2C is **IMPLEMENTED**: `1.130` is the sole crossing story point. Intro remains the sole owner of transient `playerEnteredRing` / `monkeySettled` state and reports an explicit `crossingComplete` condition with either crossing event. `COMPLETE_IF` resolves to local `STAY` for the first fact and Spine-driven `COMPLETE` for the second, when both facts are true; only completion emits `BEGIN_GLYPH_FREE_EXPLORE`. Technical points `1.130.1` and `1.130.2` and their persistent crossing milestones are removed.
