...
- The gyro control semantics are now split into `PREVIEW → COMMAND → CURRENT`: `PREVIEW` continuously follows the neutralized left-grip orientation after the first no-jump reference capture, `COMMAND` is the last drive target copied from `PREVIEW` only while the left trigger is held, and `CURRENT` is the actual `VrTiltableFloorRoot` orientation moving toward `COMMAND`. The compatible target API still represents `COMMAND`.
- `GIMBAL_CURRENT` continues to display `CURRENT`. `GIMBAL_TARGET` now displays `PREVIEW`, so inner rings 2/3 plus `PIV_TARGET_AXIS` / `srodek` can search ahead with the hand even while the platform is still traveling to a frozen `COMMAND`; `srodek` remains a pure inherited preview-axis child with no procedural runtime rotation. Rings 2/3 drift with full authored idle weight whenever the trigger is off and smoothly blend to zero animation weight only while drive/trigger is active, without restarting clip time.
- `CURRENT` floor motion still uses the existing capped exponential response (`response = 2.5`, `maxAngularSpeedDegrees = 55`), so distant movement is speed-limited while near-target convergence remains soft. Angular acceleration / heavy platform inertia is not implemented in this stage. `playerRig`, camera, locomotion reference frame, furnace progression and the panel `x/6` material preview are unchanged.
