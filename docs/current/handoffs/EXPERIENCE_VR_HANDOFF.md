...
- The target visual keeps inner rings 2/3 plus `PIV_TARGET_AXIS` / `srodek` under `GIMBAL_TARGET`: rings 2/3 drift with full authored idle weight in IDLE/LOCKED and smoothly blend to zero animation weight while clutching/TARGETING, without restarting clip time; `srodek` remains a pure inherited target-axis child with no procedural runtime rotation. The left trigger acts as a quaternion clutch for the visual `VrTiltableFloorRoot` radar only; `playerRig`, camera, locomotion reference frame, furnace progression and the panel `x/6` material preview are unchanged.
