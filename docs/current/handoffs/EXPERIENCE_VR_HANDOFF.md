...
R3b/R3c provide the shared `PlatformEnergyVfxActor` reveal foundation and separate WORLD creation one-shot. The same bounded reusable ribbon pool and midpoint/fractal generator now also serve `SECTOR_ACQUISITION` and `FLOOR_DRIVE`. Every active profile has asymmetric longitudinal width, a bright narrow core plus soft additive halo, bounded spawn variation, seeded pseudo-flicker and shallow 3D surface lift. Main bolts may create bounded one-generation short branches from exact internal main-path points; branches use ordinary shared-pool slots, never recurse and fail soft at saturation. Slots allocate geometry/material only at actor construction and do not recompute bounding spheres per spawn. Main endpoints remain exact, including authored Binder presentation targeting. Independent multilayer shells, guaranteed retry of a pool-exhausted final pulse, `RUNE_INSTALL`, detent sparks, installation/detent/motion audio, target response/field lensing and the later roadmap remain unimplemented.
...
Implemented composition now updates hand mode, R2A acquisition, R2B sector control, then the existing global gyro owner. R2B supports only EARTH, WOOD and FIRE. Their physical projection is corrected from the earlier mirrored runtime: EARTH/WOOD are mirrored downward-folding side wings on the actual outer radial edges at `±36°` from local `+Z`; both axes pass through platform origin, so MotionRoot.position stays zero while the inner edges toward FIRE descend. FIRE retains its central downward pitch on the bounds-derived inner radial hinge. The complete MotionRoot subtree remains rigid; sector visual, moving panel-3 beam anchor/overlay, energy mount, Rune Installation Frame, Zwornik and installed Rune inherit one transform without correction offsets. LEVEL 0 stays identity, while levels, gesture direction, constant-speed detents and Field semantics remain unchanged. Sector-local reveal and floor-drive VFX inherit motion automatically; R4 observes R2B `DETENT_COMMITTED` and installed Rune truth without a frame update; its immutable descriptor remains unchanged during transient DRIVING. Detent VFX, field audio, grip beam, target selection/scoring/response, lensing and METAL/WATER movement/contribution remain explicit follow-up work.
