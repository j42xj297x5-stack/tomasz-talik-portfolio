...
- `createVrCrystalReliquary` owns the always-visible, authored-scale `/glb/portal_crystal_reliquary.glb`, positioned 0.5 m from the portal toward spawn with matching orientation. Its hidden named sphere is the primary insertion target, and its named anchor is exposed for later crystal display behavior.
