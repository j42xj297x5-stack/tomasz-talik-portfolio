...
The asset and runtime ownership contracts are **IMPLEMENTED**. Both GLBs are critical P0 preloads. `VrMonkeyMotionRoot` contains only `VrMonkeyVisualRoot`/the character and Monkey Guide. `VrMonkeyStoneRoot` is created directly under `VrPlatformFixturesRoot`: it inherits platform tilt but never follows actor translation or turning. During normal composition, authored `MONKEY_STONE_ROOT` is based at platform center `(0,0,0)` and stays stationary; internal `VrMonkeyVisualRoot` alignment then makes the complete world matrices of `MONKEY_ANCHOR` and `MONKEY_SEAT_ANCHOR` equal. `(0,0,0)` is the canonical `VrMonkeyMotionRoot` transform, not a requirement that the character anchor itself occupy the world origin. No bbox centering, magic offset or final snap is involved.
