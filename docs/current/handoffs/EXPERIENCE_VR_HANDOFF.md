...
- `createVrCrystalReliquary` owns the always-visible, authored-scale `/glb/portal_crystal_reliquary.glb`. Its base is 0.5 m from the portal toward spawn and its placement root adds 1 m along the portal's quaternion-derived horizontal front toward the player. Its hidden named sphere is the primary insertion target, and its named anchor holds the visible inserted/active crystal and moves with the reliquary. The shared authored root preserves centering, grounding, and the Blender coordinate system; a single `VrReliquaryActivateButtonPlacementRoot` child offsets the separate button GLB by a 1 m radius at 45 degrees left-forward (about 0.7071 m left and forward) without modifying its authored transform.
