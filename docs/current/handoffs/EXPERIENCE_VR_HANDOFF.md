...
Status: current delivery handoff synchronized after S1, M2.1, M2.2A, M2.2B, M2.2C, M1.20R, M1.20F, M1.20M and the later floor-visibility ownership correction. It intentionally does not duplicate runtime, audio or narrative contracts.
...
- **CURRENT transition contract:** every LIVE transition authors `kind: STAY | COMPLETE | EXPLICIT | COMPLETE_IF`. `STAY` retains the point, `COMPLETE` obtains the authored successor from Spine, and `EXPLICIT` requires a concrete authored local/exit/loop target. `COMPLETE_IF` is the deliberately narrow conditional-completion mechanism for the explicit `crossingComplete` condition: false resolves to `STAY`, while true resolves to Spine-driven `COMPLETE`. It is not a general rules engine, DSL, expression system, or generic condition engine. Director publishes the resolved `transitionKind`; Runtime only forwards the accepted change and executes effects. Missing/unknown kinds and invalid kind/target combinations fail Scenario validation.
...
- M2.2B is **IMPLEMENTED**: WHERE, FOLLOW pause/resume and BEYOND are targetless `STAY` reactions in canonical points `1.100`, `1.110` and `1.120`. Their former three technical point IDs are removed from the LIVE identifier set and Scenario without aliases. GO/CROSS remain targetless `COMPLETE`; both EARLY EXIT choices remain `EXPLICIT → 100.10`.
- Runtime still delegates `CONTINUE_INTRO_INVITATION`, `APPLY_FOLLOW_PAUSE_STATE` and `CONTINUE_THRESHOLD_CHOICE` to the Intro actor. The actor remains owner of dialogue queues, choice UI, phase/timers, Monkey movement and transient `walkingPaused`; no new milestone or Director state was introduced.
- **CURRENT crossing:** `1.130` is the sole crossing story point. Intro remains the sole owner of transient `playerEnteredRing` / `monkeySettled` state and reports an explicit `crossingComplete` condition with either crossing event. `COMPLETE_IF` resolves to local `STAY` for the first fact and Spine-driven `COMPLETE` for the second, when both facts are true; only completion emits `BEGIN_GLYPH_FREE_EXPLORE`. Technical points `1.130.1` and `1.130.2` and their persistent crossing milestones are removed.
- M2.2C2 is **IMPLEMENTED**: glyph and reliquary timeouts are local `STAY + effect` reactions in canonical points `2.10`, `2.30`, and `2.40`. The former hint-only technical points are removed without aliases. Intro owns glyph hint transient state; `createVrReliquaryHints` owns reliquary `fired` / `shown` / `phase` state. The `5/5` condition point, arbitrary start, hydration, and reconstruction-backed QA aliases remain **NOT IMPLEMENTED**.
...
- LIVE first-loop points are `2.10`, `2.20`, `2.30`, and `2.40`. Old `1.140`–`1.180.1` addresses are **RETIRED / SUPERSEDED**, never runtime aliases.
- S1 authored `VR_EXPERIENCE_SCENARIO_SPINE`, added Scenario `settledConsequences` and implemented pure exclusive reconstruction. M2.1 made normal LIVE mainline routing Spine-driven: targetless completion uses authored next, while explicit `transition.target` remains only for local/exit routing. The Spine ends at LIVE `2.40`; EARLY EXIT `100.10` is outside it. Arbitrary start, hydration, owner restore APIs and reconstruction-backed aliases are **TARGET / NOT IMPLEMENTED**.
...
Migracja reindexu zmieniła wyłącznie adresy punktów i jawne targety. Eventy, numeric choices, effects, milestones, actor/runtime behavior i SG statuses są bez zmian. M1.12 ma **HARDWARE PASS — Meta Quest 3S**, SG-036 **MIGRATED**, a SG-041 jest **MIGRATED** po M1.13. Późniejszy S1 wdrożył authored Spine, `settledConsequences` i pure exclusive reconstruction, a M2.1 przeniósł normalny mainline routing Directora na Spine; explicit targets pozostały tylko dla tras local/exit. Canonical Story Reindex jest **IMPLEMENTED / behavior-neutral**; post-reindex regression: **PASS — Meta Quest 3S**.
