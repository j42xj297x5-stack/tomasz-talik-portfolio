...
The existing `1.0 s` dwell and powered-Rune filter remain authoritative. Its geometric source is exclusively the five invisible flat sector buttons derived from authored BASE geometry and held under ActorRoot above MotionRoot; local sector tilt cannot move them, while global platform tilt still does. The presentation-only beam continues to terminate at the real moving panel-3 anchor rather than at the flat button. Lightning visual branching/upgrade, detent sparks, audio, `RUNE_INSTALL` VFX/audio, Field Presentation and target response remain separate **NOT IMPLEMENTED** work.
