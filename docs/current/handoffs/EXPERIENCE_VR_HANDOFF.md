...
The asset and runtime ownership contracts are **IMPLEMENTED**. Both GLBs are critical P0 preloads. `VrMonkeyMotionRoot` contains only `VrMonkeyVisualRoot`/the character and Monkey Guide. `VrMonkeyStoneRoot` is created directly under `VrPlatformFixturesRoot`: it inherits platform tilt but never follows actor translation or turning. During normal composition, full authored anchor matrices place the stationary stone once so its `MONKEY_SEAT_ANCHOR` coincides with the character's `MONKEY_ANCHOR` at the canonical pose. No bbox centering, magic offset, final snap, or canonical-position adjustment is involved.
