...
- `createVrCrystalReliquary` owns the always-visible, authored-scale `/glb/portal_crystal_reliquary.glb`, positioned 0.5 m from the portal toward spawn with matching orientation. Its hidden named sphere is the primary insertion target, and its named anchor holds the visible inserted/active crystal. Its authored root also owns the separate activate-button GLB without modifying that companion's Blender transform.
