...
The rebuilt intro P0 is **IMPLEMENTED**: XR calibration leads through radial fog reveal and silence, three timed orientation messages and a persistent Y instruction that clears only when the panel actually opens, controls/pointer/trigger onboarding, invitation, `FOLLOWING`, threshold choice, physical entry into the ring, `MONKEY_SETTLING` and `GLYPH_FREE_EXPLORE`. The sequence starts `monkeyMotionRoot` at canonical radius `18` and returns it to final `(0,0,0)`. XR calibration moves only `playerRig`; it never supplies Monkey placement.
...
The asset and runtime ownership contracts are **IMPLEMENTED**. Both GLBs are critical P0 preloads. `VrMonkeyMotionRoot` contains only `VrMonkeyVisualRoot`/the character and Monkey Guide. `VrMonkeyStoneRoot` is created directly under `VrPlatformFixturesRoot`: it inherits platform tilt but never follows actor translation or turning. During normal composition, authored `MONKEY_STONE_ROOT` is based at platform center `(0,0,0)` and stays stationary; internal `VrMonkeyVisualRoot` alignment makes the world position and rotation of `MONKEY_ANCHOR` match `MONKEY_SEAT_ANCHOR`, while preserving the character's authored GLB scale. The seat defines pose, not character size. `(0,0,0)` is the canonical `VrMonkeyMotionRoot` transform, not a requirement that the character anchor itself occupy the world origin. No bbox centering, magic offset or final snap is involved.

The Monkey always starts at radius `18`, independently of tracked-head position. The stone stays hidden during the initial emptiness, appears with the existing glyph-ring reveal (immediately in QA bypass), and remains at the ring center while the Monkey follows, crosses and settles back to canonical. The character-only interaction root excludes the stone from Monkey rays and halo. The persistent Y prompt still clears only after the player guide is actually open.

Monkey Guide attention, message and dialogue transforms now have one actor-local settings contract; their final clearance above the authored full-scale Monkey requires Meta Quest hardware QA.
