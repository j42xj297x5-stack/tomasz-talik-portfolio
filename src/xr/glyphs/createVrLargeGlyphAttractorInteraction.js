...
export function createVrLargeGlyphAttractorInteraction({ controllers, nodes, glyphOrbit, handModeController,
  ...
  if (!Array.isArray(controllers) || !Array.isArray(nodes)) throw new TypeError('controllers and nodes must be arrays.');
  if (!glyphOrbit?.suspendNode || !glyphOrbit?.getCanonicalTransform || !glyphOrbit?.resumeNode)
    throw new TypeError('glyphOrbit must expose transient ownership API.');
  ...
  function beginReturn(node) { if (!node || returning?.node === node) return;
    ...
    returning = { node, startPosition: node.position.clone(), startQuaternion: node.quaternion.clone(), elapsed: 0 };
  ...
  function updateReturn(delta) { if (!returning) return; const canonical = glyphOrbit.getCanonicalTransform(returning.node);
    if (!canonical) throw new Error('Large glyph lost its canonical orbit slot.');
    returning.elapsed += delta; const progress = clamp01(returning.elapsed / settings.returnDuration);
    const eased = progress * progress * (3 - 2 * progress);
    returning.node.position.lerpVectors(returning.startPosition, canonical.position, eased);
    returning.node.quaternion.slerpQuaternions(returning.startQuaternion, canonical.quaternion, eased);
    returning.node.scale.copy(canonical.scale);
    if (progress === 1) { const node = returning.node; returning = null; states.set(node, STATE.ORBIT); glyphOrbit.resumeNode(node); }
  }
  function update(deltaSeconds = 0) {
    ...
    if (active) {
      ...
      if (states.get(active) === STATE.CAPTURED || distance <= settings.captureRadius) {
        states.set(active, STATE.CAPTURED); setWorldPosition(active, anchor); active.scale.copy(glyphOrbit.getCanonicalTransform(active).scale);
      ...
      setWorldPosition(active, position); active.scale.copy(glyphOrbit.getCanonicalTransform(active).scale);
    ...
    if (target && primaryAction > settings.triggerThreshold && canPullLargeGlyphs() === true) {
      active = target; setTarget(null); if (!glyphOrbit.suspendNode(active)) { active = null; return; }
  ...
  function reset() { const leased = active ?? returning?.node; if (leased) { onPullCancel({ target: leased }); glyphOrbit.resumeNode(leased);
