...
export function createVrLargeGlyphAttractorInteraction({ controllers, largeGlyphActor, handModeController,
  ...
  const familyCodes = new Map(nodes.map((node) => {
    const protoAstro = resolveVrPageProtoAstro({ glyphId: node.userData.id });
    if (!protoAstro) throw new Error(`Cannot resolve canonical Large Glyph identity: ${node.userData.id ?? 'unknown'}`);
    return [node, protoAstro.descriptor.familyCode];
  }));
  ...
  const isFamilyEligible = (node) => isFamilyTargetable(familyCodes.get(node)) === true;
  ...
  function update(deltaSeconds = 0) {
    ...
    if (isHigherPriorityInteractionActive(right) === true) { setTarget(null);
