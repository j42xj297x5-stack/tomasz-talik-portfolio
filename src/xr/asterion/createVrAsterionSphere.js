...
import { computeGimbalLocalQuaternion } from './asterionGyroMath.js';

const REQUIRED_NODES = Object.freeze(['ASTERION_ROOT', 'GIMBAL_CURRENT', 'GIMBAL_TARGET']);
...
export function createVrAsterionSphere({ model, animations = [], settings, enabled = false, debug = false }) {
  ...
  const conflictingClips = animations.filter((clip) => animationTargetsRuntimeNode(clip, REQUIRED_NODES));
  ...
  const playableIdleClips = idleClips.filter((clip) => !animationTargetsRuntimeNode(clip, REQUIRED_NODES));
  ...
  function equipTo(record) {
    ...
    object.visible = true;
    if ((debug || enabled) && !diagnosticsLogged) {
      diagnosticsLogged = true;
      console.info('[AsterionSphere]', { clipNames: animations.map((clip) => clip.name), idleClipCount: idleClips.length,
        requiredNodes: Object.fromEntries(REQUIRED_NODES.map((name) => [name, Boolean(requiredNodes[name])])),
  ...
  function syncGimbals({ currentQuaternion, targetQuaternion, worldRoot }) {
    ...
    const syncOne = (node, floorLocalQuaternion) => {
      if (!node) return;
      const desiredWorldQuaternion = rootWorldQuaternion.clone().multiply(floorLocalQuaternion);
      const parentWorldQuaternion = node.parent?.getWorldQuaternion(new THREE.Quaternion()) ?? new THREE.Quaternion();
      node.quaternion.copy(computeGimbalLocalQuaternion({ desiredWorldQuaternion, nodeParentWorldQuaternion: parentWorldQuaternion }));
    };
    syncOne(requiredNodes.GIMBAL_CURRENT, currentQuaternion);
    syncOne(requiredNodes.GIMBAL_TARGET, targetQuaternion);
  }
