...
function updateNodeHoverAnimation(node, elapsed) {
  ...
  const duration = prefersReducedMotion ? REDUCED_MOTION_HOVER_ANIMATION_DURATION : HOVER_ANIMATION_DURATION;
...
export function createGlyphVisualNode(item, { assetManager = null } = {}) {
  ...
  if (item.modelPath) {
    ...
    if (model) {
      ...
      console.info(`[createGlyphVisualNode] Attached node model for ${item.id} from AssetManager cache.`);
    } else {
      console.info(`[createGlyphVisualNode] Sphere fallback retained for ${item.id} because the glyph model was not in AssetManager cache.`);
    }
  }
  return node;
}
