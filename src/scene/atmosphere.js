...
export function createBackgroundAtmosphere(configOverrides = {}) {
  ...
  function applySmallGlyphMaterial(){ const s=config.smallGlyphRelics; smallGlyphRelicStates.forEach(({object})=>object.traverse((child)=>{ if(!child.isMesh) return; (Array.isArray(child.material)?child.material:[child.material]).forEach((m)=>{m.transparent=true;m.opacity=s.debugVisible?1:s.opacity;m.needsUpdate=true;}); })); smallGlyphRelicsGroup.visible=Boolean(config.enabled&&s.enabled); }
