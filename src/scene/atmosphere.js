...
export function createBackgroundAtmosphere(configOverrides = {}) {
  ...
  function applyShellMaterial(){ const s=config.shellRelics; shellRelicStates.forEach(({object})=>object.traverse((child)=>{ if(!child.isMesh) return; (Array.isArray(child.material)?child.material:[child.material]).forEach((m)=>{m.transparent=true;m.opacity=s.debugVisible?1:s.opacity;m.needsUpdate=true;}); })); shellRelicsGroup.visible=Boolean(config.enabled&&s.enabled); }

  async function rebuild(){ while(root.children.length>0) root.remove(root.children[0]); dustField=null; helperGroup=null; clearRelics(); clearShellRelics(); if(!config.enabled) return; if(config.dust.enabled){dustField=createDustField(config);root.add(dustField.points);applyDustMaterialOptions();} helperGroup=createShellDebugHelpers(config);root.add(helperGroup);setHelpersVisible(); await loadRelicModels(); await loadShellModels(); root.add(stoneRelicsGroup); root.add(shellRelicsGroup); rebuildStoneRelics(); rebuildShellRelics(); applyStoneMaterial(); applyShellMaterial(); }
  function applySettings(next={},type='rebuild'){ config=resolveAtmosphereConfig({ ...config, ...next }); if(type==='material'){applyDustMaterialOptions();applyStoneMaterial();applyShellMaterial();return;} if(type==='helpers'){setHelpersVisible();return;} if(type==='stone-runtime'){applyStoneMaterial();return;} if(type==='stone-rebuild'){rebuildStoneRelics();applyStoneMaterial();return;} if(type==='shell-runtime'){applyShellMaterial();return;} if(type==='shell-rebuild'){rebuildShellRelics();applyShellMaterial();return;} void rebuild(); }

  void rebuild();
  return { object3d: root, applySettings, rebuild, update(deltaSeconds = 0){ if(!root.parent) return; if(dustField) root.rotation.y += config.dust.rotationSpeed*deltaSeconds; stoneRelicsGroup.rotation.y += config.stoneRelics.orbitSpeed*deltaSeconds; shellRelicsGroup.rotation.y += config.shellRelics.orbitSpeed*deltaSeconds; stoneRelicStates.forEach((r,i)=>{r.object.rotation.x += r.spin.x*deltaSeconds; r.object.rotation.y += r.spin.y*deltaSeconds; r.object.rotation.z += r.spin.z*deltaSeconds; const drift=Math.sin(performance.now()*0.0002+i)*0.01; r.object.position.copy(r.basePosition).addScaledVector(r.basePosition.clone().normalize(),drift);}); shellRelicStates.forEach((r,i)=>{r.object.rotation.x += r.spin.x*deltaSeconds; r.object.rotation.y += r.spin.y*deltaSeconds; r.object.rotation.z += r.spin.z*deltaSeconds; const drift=Math.sin(performance.now()*0.00017+i)*0.008; r.object.position.copy(r.basePosition).addScaledVector(r.basePosition.clone().normalize(),drift);}); } };
}
