...
export function createBackgroundAtmosphere(configOverrides = {}, { assetManager = null } = {}) {
  ...
  function applyStoneMaterial(){ const s=config.stoneRelics; stoneRelicStates.forEach(({object})=>object.traverse((child)=>{ if(!child.isMesh) return; (Array.isArray(child.material)?child.material:[child.material]).forEach((m)=>{m.transparent=true;m.opacity=(s.debugVisible?1:s.opacity)*(progressionMultipliers.stones ?? 1);m.needsUpdate=true;}); })); stoneRelicsGroup.visible=Boolean(config.enabled&&s.enabled); }
  ...
  function applySmallGlyphMaterial(){ const s=config.smallGlyphRelics; smallGlyphRelicStates.forEach(({object})=>object.traverse((child)=>{ if(!child.isMesh) return; (Array.isArray(child.material)?child.material:[child.material]).forEach((m)=>{m.transparent=true;m.opacity=(s.debugVisible?1:s.opacity)*(progressionMultipliers.smallGlyphs ?? progressionMultipliers.miniGlyphs ?? 1);m.needsUpdate=true;}); })); smallGlyphRelicsGroup.visible=Boolean(config.enabled&&s.enabled); }
