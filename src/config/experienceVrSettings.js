...
export const DEFAULT_EXPERIENCE_VR_SETTINGS = Object.freeze({
  ...
  asterionSectorBeam: {
    ...
    segments: 18,
    radialSegments: 8,
    startRadiusMeters: 0.0015,
    endRadiusMeters: 0.065,
    arcHeightFactor: 0.08,
    opacity: 0.78,
    rainbowStrength: 0.6,
    terminalBloomScale: 1.1,
    terminalBloomOpacity: 0.35,
  ...
  runeStoneSpatialAudio: { maxDistanceMeters: 2.0, refDistanceMeters: 0.25 },
...
export function normalizeExperienceVrSettings(candidate) {
  ...
  return {
    ...
    asterionSectorBeam: {
      ...
      radialSegments: Math.round(finiteNumber(candidate.asterionSectorBeam?.radialSegments, defaults.asterionSectorBeam.radialSegments, { min: 3, max: 16 })),
      startRadiusMeters: finiteNumber(candidate.asterionSectorBeam?.startRadiusMeters, defaults.asterionSectorBeam.startRadiusMeters, { min: 0.0005, max: 0.05 }),
      endRadiusMeters: finiteNumber(candidate.asterionSectorBeam?.endRadiusMeters, defaults.asterionSectorBeam.endRadiusMeters, { min: 0.005, max: 0.25 }),
      ...
      terminalBloomScale: finiteNumber(candidate.asterionSectorBeam?.terminalBloomScale, defaults.asterionSectorBeam.terminalBloomScale, { min: 0.5, max: 2 }),
      terminalBloomOpacity: finiteNumber(candidate.asterionSectorBeam?.terminalBloomOpacity, defaults.asterionSectorBeam.terminalBloomOpacity, { min: 0, max: 1 }),
    ...
    runeStoneInstallation: {
      handoffRadiusMeters: finiteNumber(candidate.runeStoneInstallation?.handoffRadiusMeters,
        defaults.runeStoneInstallation.handoffRadiusMeters, { min: 9.01, max: 60 }),
      hoverHeightMeters: finiteNumber(candidate.runeStoneInstallation?.hoverHeightMeters,
        defaults.runeStoneInstallation.hoverHeightMeters, { min: Number.EPSILON, max: 20 }),
      phaseDurationSeconds: finiteNumber(
    ...
    runeStoneSpatialAudio: (() => {
      ...
      const refDistanceMeters = finiteNumber(candidate.runeStoneSpatialAudio?.refDistanceMeters,
        defaults.runeStoneSpatialAudio.refDistanceMeters, { min: Number.EPSILON, max: maxDistanceMeters - Number.EPSILON });
    ...
    platformEnergyVfx: {
      ...
      acquisitionStrengthMin: finiteNumber(candidate.platformEnergyVfx?.acquisitionStrengthMin, defaults.platformEnergyVfx.acquisitionStrengthMin, { min: 0, max: 2 }),
      acquisitionStrengthMax: finiteNumber(candidate.platformEnergyVfx?.acquisitionStrengthMax, defaults.platformEnergyVfx.acquisitionStrengthMax, { min: 0, max: 2 }),
      driveSpawnIntervalSeconds: finiteNumber(candidate.platformEnergyVfx?.driveSpawnIntervalSeconds, defaults.platformEnergyVfx.driveSpawnIntervalSeconds, { min: 0.02, max: 1 }),
    ...
    observationWindow: {
      durationSeconds: finiteNumber(candidate.observationWindow?.durationSeconds,
        defaults.observationWindow.durationSeconds, { min: 0.1, max: 60 })
    },
    intro: {
      ...
      followGraceDistance: finiteNumber(candidate.intro?.followGraceDistance,
        defaults.intro.followGraceDistance, { min: 0, max: 20 }),
