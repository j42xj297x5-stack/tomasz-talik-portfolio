...
assert.equal(crystal.object.position.y, 1.2);
assert.equal(monkeyRoot.position.length(), 18); assert.equal(Math.hypot(crystal.object.position.x, crystal.object.position.z), 19); assert.equal(playerStart.length(), 20);
assert.ok(crystal.object.position.clone().setY(0).sub(monkeyRoot.position).dot(playerStart.clone().sub(crystal.object.position.clone().setY(0))) > 0,
...
held = true; crystal.object.position.set(0, 1.2, 18.51); tutorial.update(0); assert.equal(handoffs, 0);
