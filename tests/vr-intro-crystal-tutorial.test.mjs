...
const tutorial = createVrIntroCrystalTutorial({
  ...
  monkeyRoot, getPlayerPosition: () => new THREE.Vector3(0, 1.7, 2), crystalCollection: collection,
  crystalDefinition: { asset: 'water-1' }, locale: 'pl',
  settings: { spawnDistanceFromMonkey: 1, handoffDistanceFromMonkey: 0.5, messageDisplayDuration: 1 },
...
assert.ok(crystal.object.position.distanceTo(new THREE.Vector3(0, 0, 1)) < 1e-8, 'crystal spawns one metre from Monkey toward player');
...
held = true; crystal.object.position.set(0, 0, 0.51); tutorial.update(0); assert.equal(handoffs, 0);
crystal.object.position.z = 0.5; tutorial.update(0); tutorial.update(1); assert.equal(handoffs, 1, 'threshold emits once');
