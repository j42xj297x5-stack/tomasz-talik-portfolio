...
gltfPlaque.show({ title: 'Waiting', body: 'Three seconds' }, { duration: 3, animateScale: false });
assert.deepEqual(gltfSurface.scale.toArray(), originalScale.toArray());
gltfPlaque.update(1.5);
assert.ok(Math.abs(gltfSurface.material.opacity - 0.5) < 1e-8);
gltfPlaque.update(1.5);
assert.equal(gltfSurface.material.opacity, 1);
gltfPlaque.reset();
