...
function fixture({ bypass = false, onReliquaryReveal = () => {} } = {}) {
  ...
  const largeGlyphObject = new THREE.Group(); const largeGlyphActor = { object: largeGlyphObject, setPresentationVisible(value) { largeGlyphObject.visible = value; return true; } }; const platformFixturesRoot = new THREE.Group(); const sector = new THREE.Group(); sector.userData.branchId = 'x'; progressFloor.object.add(sector);
...
assert.equal(f.monkeyStoneRoot.visible, true); assert.equal(f.largeGlyphObject.visible, true); assert.equal(f.fog.radius, 20);
