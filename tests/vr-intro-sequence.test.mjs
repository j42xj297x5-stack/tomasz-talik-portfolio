...
function fixture({ bypass = false, lineCounts = {} } = {}) {
  const monkeyMotionRoot = new THREE.Group();
  monkeyMotionRoot.position.set(0.25, 1.25, -0.5);
  ...
  const trackedHead = new THREE.Group(); trackedHead.position.set(0.75, 1.7, -0.5); playerRig.add(trackedHead);
  ...
  let interactionEnabled = true; let emergeAlpha = 1; let raysReady = 0;
  const monkeyGuide = { showMessage(value) { message = value; messages.push(value); return { lineCount: value ? (lineCounts[value] ?? 1) : 0 }; }, setDialogueOverride(value) { override = value; },
  ...
  const settings = { enabled: true, locale: 'en', playerStartRadius: 20, monkeyStartRadius: 18,
    thresholdStopOutsideDistance: 1, guideSpeed: 1, guideTurnDuration: 1, followGraceDistance: 3, pauseDistance: 3.2,
  ...
  const sequence = createVrIntroSequence({ monkeyGuide, monkeyMotionRoot, playerRig, glyphRing, progressFloor,
    ...
    getHeadPosition: () => trackedHead.getWorldPosition(new THREE.Vector3()),
    onOpeningRaysReady: () => { raysReady += 1; },
    setMonkeyEmergeAlpha(value) { emergeAlpha = value; },
    bypass, onEndSession: () => { ended += 1; } });
  return { sequence, monkeyMotionRoot, visualRoot, playerRig, trackedHead, glyphRing, platformFixturesRoot, sector,
    ...
    getEmergeAlpha: () => emergeAlpha, getRaysReady: () => raysReady };
}

const intro = fixture();
assert.equal(intro.sequence.getState(), VR_INTRO_STATE.XR_CALIBRATING);
...
assert.equal(intro.monkeyMotionRoot.position.y, 1.25, '18 m start preserves canonical platform-local height');
assert.equal(intro.sequence.getDebugSnapshot().xrCalibrated, false);
...
intro.sequence.update(6); assert.equal(intro.sequence.getState(), VR_INTRO_STATE.XR_CALIBRATING);
assert.equal(intro.getEmergeAlpha(), 0, 'emergence is frozen before XR calibration');
intro.sequence.beginAfterXrCalibration();
assert.equal(intro.sequence.getDebugSnapshot().xrCalibrated, true);
...
assert.equal(intro.sequence.getState(), VR_INTRO_STATE.XR_CALIBRATING); assert.equal(intro.getEmergeAlpha(), 0);
intro.sequence.beginAfterXrCalibration();
...
assert.equal(intro.getRaysReady(), 0, 'rays remain gated throughout Good display');
...
assert.equal(intro.getRaysReady(), 0, 'rays remain gated throughout the first gap');
intro.sequence.update(0.01); assert.equal(intro.getMessage(), VR_INTRO_COPY.en.opening[1]);
assert.equal(intro.getRaysReady(), 1, 'rays open immediately before You have hands');
intro.sequence.reset();
...
assert.equal(intro.getMessage(), '', 'reset during GAP removes the transitional timer'); intro.sequence.beginAfterXrCalibration(); intro.sequence.update(12);
...
intro.trackedHead.position.copy(intro.monkeyMotionRoot.position); intro.sequence.update(0.5);
...
intro.trackedHead.position.z = 30; intro.playerRig.position.copy(intro.monkeyMotionRoot.position);
intro.sequence.update(2.49);
assert.equal(intro.sequence.isGuidePaused(), false, 'first 3 m ignore follow distance');
assert.notEqual(intro.getMessage(), VR_INTRO_COPY.en.going, 'grace walk has no reminder');
intro.sequence.update(0.01); intro.sequence.update(0.01);
assert.equal(intro.sequence.isGuidePaused(), true); const pausedZ = intro.monkeyMotionRoot.position.z;
assert.equal(intro.getMessage(), VR_INTRO_COPY.en.going, 'pause shows a non-decision reminder');
intro.playerRig.position.z = intro.monkeyMotionRoot.position.z;
intro.sequence.update(1); assert.equal(intro.monkeyMotionRoot.position.z, pausedZ, 'rig alone cannot resume tracked-head following');
intro.playerRig.position.set(0, 0, 0); intro.trackedHead.position.copy(intro.monkeyMotionRoot.position); intro.sequence.update(1);
assert.equal(intro.sequence.isGuidePaused(), false, 'resume threshold provides hysteresis');
assert.equal(intro.getMessage(), '', 'tracked-head approach clears reminder automatically');
for (let i = 0; i < 20 && intro.sequence.getState() === VR_INTRO_STATE.FOLLOWING; i += 1) {
  intro.trackedHead.position.copy(intro.monkeyMotionRoot.position);
...
assert.ok(intro.monkeyMotionRoot.position.distanceTo(new THREE.Vector3(0, 1.25, 5)) < 1e-6,
  'threshold is based on PlatformOrigin, independently of canonical root offset');
assert.equal(intro.monkeyMotionRoot.position.y, 1.25, 'threshold preserves canonical height');
...
intro.playerRig.position.set(0, 0, 0); intro.trackedHead.position.set(0, 1.7, 3.9); intro.sequence.update(0.1);
...
assert.equal(intro.monkeyMotionRoot.position.y, 1.25, 'crossing preserves canonical height');
...
intro.sequence.reset(); assert.equal(intro.sequence.getState(), VR_INTRO_STATE.XR_CALIBRATING, 'session reset is deterministic');
...
for (const lineCount of [1, 2, 3]) {
  const timed = fixture({ lineCounts: { [VR_INTRO_COPY.en.opening[0]]: lineCount } });
  timed.sequence.beginAfterXrCalibration(); timed.sequence.update(12);
  timed.sequence.update(lineCount * 2 - 0.01);
  assert.equal(timed.getMessage(), VR_INTRO_COPY.en.opening[0], `${lineCount} rendered lines remain for ${lineCount * 2} s`);
  timed.sequence.update(0.01); assert.equal(timed.getMessage(), '');
  timed.sequence.update(0.49); assert.equal(timed.getMessage(), '', 'line timing does not alter the 0.5 s gap');
  timed.sequence.update(0.01); assert.equal(timed.getMessage(), VR_INTRO_COPY.en.opening[1]);
}
...
bypassed.sequence.beginAfterXrCalibration();
assert.equal(bypassed.sequence.getState(), VR_INTRO_STATE.BYPASSED, 'calibration does not start the bypassed intro');
assert.equal(bypassed.sequence.getDebugSnapshot().xrCalibrated, true);
