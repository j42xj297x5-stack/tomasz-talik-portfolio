...
function fixture({ bypass = false, onReliquaryReveal = () => {} } = {}) {
  ...
  const monkeyVisualRoot = new THREE.Group();
...
assert.equal(f.locomotion.resetCalls, 0, 'Intro reset does not call the locomotion owner reset');
f.glyphRing.visible = false; f.monkeyStoneRoot.visible = false; f.monkeyVisualRoot.visible = false; f.platformFixturesRoot.visible = false;
f.sequence.reset();
assert.deepEqual([f.glyphRing.visible, f.monkeyStoneRoot.visible, f.monkeyVisualRoot.visible, f.platformFixturesRoot.visible],
  [false, false, false, false], 'Intro reset preserves presentation state owned by external domains');
f.glyphRing.visible = true; f.monkeyStoneRoot.visible = true; f.monkeyVisualRoot.visible = true; f.platformFixturesRoot.visible = true;
...
assert.deepEqual(f.getMonkeySettledPayloads(), [{ crossingComplete: true }], 'Monkey-second event reports crossing completion');
...
assert.equal(early.sequence.getState(), VR_INTRO_STATE.WAIT_RUNTIME_AFTER_RELIQUARY_REVEAL, 'fact emission waits for Runtime and does not restore free explore');
early.sequence.update(100); assert.equal(early.getReliquaryRevealCompleted(), 1, 'completion fact emits at most once');
assert.equal(early.sequence.completeReliquaryReveal(), true, 'Runtime continuation restores the legacy free-explore state');
assert.equal(early.sequence.getState(), VR_INTRO_STATE.GLYPH_FREE_EXPLORE);
...
assert.deepEqual(lateEntry.getMonkeySettledPayloads(), [{ crossingComplete: false }], 'Monkey-first event reports a local STAY condition');
...
assert.deepEqual(lateEntry.getPlayerEntryPayloads(), [{ crossingComplete: true }], 'player-second event reports crossing completion');
