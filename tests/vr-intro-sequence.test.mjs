...
f.sequence.chooseThreshold('cross'); f.head.set(0, 1.7, 7.59); const before = f.playerRig?.position?.clone?.(); f.sequence.update(.1); assert.ok(f.fog.radius < 6); assert.equal(f.sequence.getDebugSnapshot().playerSafelyInside, false); assert.equal(f.getRadius(), Infinity);
