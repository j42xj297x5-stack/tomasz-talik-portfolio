...
assert.ok(intro.monkeyAnchor.position.distanceTo(intro.canonicalMonkeyPosition.clone().add(new THREE.Vector3(0, 0, 18))) < 1e-9,
  'canonical model/pivot offset is preserved beneath the logical 18 m radius');
assert.equal(intro.playerRig.position.z - (intro.monkeyAnchor.position.z - intro.canonicalMonkeyPosition.z), 2,
...
assert.ok(intro.monkeyAnchor.position.distanceTo(intro.canonicalMonkeyPosition.clone().add(new THREE.Vector3(0, 0, 5))) < 1e-6,
