...
portalObject.position.set(2, 0, -3);
...
stone.material.transparent = true; stone.material.opacity = 0.4;
...
const reliquary = createVrCrystalReliquary({
  parent: scene, portalAnchor: portalObject, reliquaryModel: source,
...
assert.equal(reliquary.insertFeedback.visible, false, 'insert feedback is hidden initially');
assert.equal(reliquary.insertFeedback.material.depthWrite, false);
...
reliquary.object.traverse((child) => { if (child.isMesh && child !== insertZone) (Array.isArray(child.material) ? child.material : [child.material]).forEach((material) => revealMaterials.push(material)); });
assert.ok(revealMaterials.length > 0);
assert.ok(revealMaterials.every((material) => material.opacity === 0), 'model and attached button start transparent');
const originalOpacity = new Map(revealMaterials.map((material) => [material, material.userData.vrReliquaryOriginalOpacity]));
reliquary.reset();
reliquary.reveal(3); reliquary.update(1.5);
originalOpacity.forEach((opacity, material) => assert.ok(Math.abs(material.opacity - opacity * .5) < 1e-9));
assert.equal(reliquary.isInteractionEnabled(), false);
reliquary.update(1.5);
originalOpacity.forEach((opacity, material) => assert.ok(Math.abs(material.opacity - opacity) < 1e-9));
assert.equal(reliquary.isInteractionEnabled(), true);
...
const portalPosition = portalObject.position.clone();
const portalForward = new THREE.Vector3(0, 0, 1).applyQuaternion(portalObject.quaternion).setY(0).normalize();
assert.ok(reliquary.object.position.distanceTo(portalPosition.clone().addScaledVector(portalForward, 1.5)) < 1e-10);
assert.equal(reliquary.object.position.y, 0);
assert.ok(Math.abs(reliquary.object.rotation.y - THREE.MathUtils.degToRad(5.41)) < 1e-12);
...
assert.equal(reliquary.setInsertFeedback('VALID'), true);
assert.equal(reliquary.insertFeedback.visible, true);
assert.equal(reliquary.insertFeedback.userData.feedbackState, 'VALID');
reliquary.setInsertFeedback('INVALID');
assert.equal(reliquary.insertFeedback.userData.feedbackState, 'INVALID');
reliquary.setInsertFeedback(null);
assert.equal(reliquary.insertFeedback.visible, false);
...
reliquary.place();
assert.ok(reliquary.object.position.distanceTo(placedOnce.clone().add(new THREE.Vector3(-3, 0, 1.25))) < 1e-10,
  'reliquary follows its portal anchor');
assert.ok(companion.getWorldPosition(new THREE.Vector3()).distanceTo(companionWorldOnce.clone().add(new THREE.Vector3(-3, 0, 1.25))) < 1e-10,
  'button companions follow the portal-anchored reliquary');
