...
assert.match(vr, /entryTransition\.reset\(\);\s*spatialPlaque\.reset\(\);\s*glyphPlaque\.reset\(\);\s*plaqueComposition\.reset\(\);\s*playerRig\.position\.set\(settings\.spawn\.position\.x/);
assert.match(vr, /function handleSessionEnd\(\)[\s\S]*plaqueComposition\.reset\(\)/);
