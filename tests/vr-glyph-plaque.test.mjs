...
assert.equal(plaque.showForGlyph(glyph), true); assert.equal(plaque.state, 'appearing'); assert.equal(plaque.object.position.y, 0); assert.ok(Math.abs(plaque.object.rotation.x) < 1e-9);
assert.ok(Math.abs(plaque.getActiveBounds().max.y - 0.225) < 1e-9);
