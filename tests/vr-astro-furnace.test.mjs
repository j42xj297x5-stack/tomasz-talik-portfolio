...
assert.equal(normalized.openButton.emissionHover, 4.5);
assert.equal(normalized.activateButton.emissionHover, 5);
assert.equal(normalized.optionButton.emissionHover, 7);
...
assert.equal(activateInteraction.canStartConstruction(), true, 'completed shell cycle is construction-capable while CLOSED');
assert.equal(activateInteraction.startConstruction(), true, 'UTWÓRZ accepts the completed shell cycle');
assert.equal(activateInteraction.getState(), ASTRO_FURNACE_PROCESS_STATES.PREPARING_CONSTRUCTION);
assert.equal(activateInteraction.getProcessKind(), ASTRO_FURNACE_PROCESS_KINDS.SHELL_EXTRACTION);
assert.equal(activateInteraction.getProgress(), 0, 'button release is outside construction progress');
assert.equal(activateInteraction.startConstruction(), false, 'duplicate UTWÓRZ cannot schedule another release or process');
assert.equal(processStarts, 1, 'create audio/process callback stays silent during button release');
activateInteraction.update(0.11);
assert.equal(activateInteraction.getState(), ASTRO_FURNACE_PROCESS_STATES.SPINUP);
assert.equal(activateInteraction.getProcessKind(), ASTRO_FURNACE_PROCESS_KINDS.ASTERION_CONSTRUCTION);
assert.equal(activateInteraction.getProgress(), 0, 'construction begins at zero after authored reverse animation');
assert.equal(processStarts, 2, 'construction emits exactly one process/audio start after release');
activateInteraction.update(0.999);
assert.notEqual(activateInteraction.getState(), ASTRO_FURNACE_PROCESS_STATES.COMPLETE, 'the complete construction duration is preserved');
activateInteraction.update(0.001);
assert.equal(activateInteraction.getState(), ASTRO_FURNACE_PROCESS_STATES.COMPLETE);
assert.equal(processStarts, 2); assert.equal(processStops, 2);
