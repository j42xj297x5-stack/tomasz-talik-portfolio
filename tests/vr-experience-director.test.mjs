...
assert.deepEqual(vrExperienceScenario.points.map(({ id }) => id), ['1.1', '1.2', '1.3', '1.4', '1.4.1', '1.4.2', '1.4.3', '1.4.4', '1.4.5', '1.4.5.1', '1.4.5.1.1', '1.4.5.1.1.1', '1.4.5.1.1.2', '1.4.5.2', '100.10']);
...
assert.equal(vrExperienceScenario.metadata.stage, 'M1_12_THRESHOLD_CHOICE_BRANCH');
...
assert.equal(productionChange.previousPointId, VR_EXPERIENCE_POINT['1.1']);
assert.equal(productionChange.currentPointId, VR_EXPERIENCE_POINT['1.2']);
...
assert.equal(new ExperienceDirector({ scenario: vrExperienceScenario }).dispatch(VR_SCENARIO_EVENT.MONKEY_TRIGGERED), null,
  'Monkey trigger is rejected before point 1.4.4');
...
assert.equal(silenceCompleteChange.currentPointId, VR_EXPERIENCE_POINT['1.4']);
...
assert.equal(guideOpenChange.currentPointId, VR_EXPERIENCE_POINT['1.4.1']);
...
assert.equal(monkeyTriggeredChange.currentPointId, VR_EXPERIENCE_POINT['1.4.5']);
...
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.MONKEY_TRIGGERED), null, 'Monkey trigger is accepted once and 1.4.5 is terminal');
...
assert.equal(goChange.currentPointId, VR_EXPERIENCE_POINT['1.4.5.1']);
...
assert.equal(thresholdChange.previousPointId, VR_EXPERIENCE_POINT['1.4.5.1']);
assert.equal(thresholdChange.currentPointId, VR_EXPERIENCE_POINT['1.4.5.1.1']);
...
assert.equal(crossChange.currentPointId, VR_EXPERIENCE_POINT['1.4.5.1.1.1']);
...
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.INTRO_INVITATION_SELECTED, { choice: 2 }).currentPointId, VR_EXPERIENCE_POINT['1.4.5.2']);
...
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.INTRO_INVITATION_SELECTED, { choice: 2 }).currentPointId, VR_EXPERIENCE_POINT['1.4.5.2'], 'choice 2 is an explicit accepted self-loop');
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.INTRO_INVITATION_SELECTED, { choice: 1 }).currentPointId, VR_EXPERIENCE_POINT['1.4.5.1']);
...
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.THRESHOLD_SELECTED, { choice: 2 }).currentPointId, VR_EXPERIENCE_POINT['1.4.5.1.1.2']);
assert.equal(productionDirector.dispatch(VR_SCENARIO_EVENT.THRESHOLD_SELECTED, { choice: 2 }).currentPointId, VR_EXPERIENCE_POINT['1.4.5.1.1.2']);
